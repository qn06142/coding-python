...
long long calc(int pos, bool contain, bool carry, bool A_leq_B, string A = "", string B = "") {
    if (pos < 0) {
        if(carry == 0 and contain) {
            debug(A, B);
        }
        return carry == 0 && contain;  
    }
    //if (dp[pos][contain][carry][A_leq_B] != -1) return dp[pos][contain][carry][A_leq_B];
    ...
    for (int ca = 0; ca <= 9; ++ca) {
        for (int cb = 0; cb <= 9; ++cb) {
            if (!A_leq_B && ca > cb) continue;  

            int sum = ca + cb + carry;  
            if (sum % 10 == digitN) {
                bool new_contain = contain || (ca == 6 || ca == 8 || cb == 6 || cb == 8);
                bool new_A_leq_B = A_leq_B || (ca < cb);  
                ans += calc(pos - 1, new_contain, sum / 10, new_A_leq_B, to_string(ca) + A, to_string(cb) + B);
            }
        }
    }
    return dp[pos][contain][carry][A_leq_B] = ans;
}

int main() {
    while (cin >> n) {
        memset(dp, -1, sizeof dp);
        cout << calc(n.size() - 1, 0, 0, 0) << '\n';  
    }
    return 0;
}