...
int n, m, overlap[M][30], dp[N][M];
string s;

int calculate_overlap(const string& prefix, const string& s) {
    int lp = prefix.length();
    for (int k = 0; k < lp; k++) {
        if (prefix.substr(k) == s.substr(0, lp - k)) {
            return lp - k;
        }
    }
    return 0;
...
void precompute_overlap() {
    for (int i = 0; i < m; i++) {
        for (int j = 0; j < 26; j++) {
            string prefix = s.substr(0, i) + char('A' + j);
            overlap[i][j] = calculate_overlap(prefix, s);
        }
    }
}
...
int solve() {
    ...
    for (int i = 0; i < n; i++) {
        for (int j = 0; j < m; j++) {
            for (int k = 0; k < 26; k++) {
                dp[i + 1][overlap[j][k]] += dp[i][j];
                dp[i + 1][overlap[j][k]] %= MOD;
            }
        }
    }
    ...
    for (int i = 0; i < n; i++) {
        total_sequences = (total_sequences * 26LL) % MOD;
    }

    int invalid_sequences = 0;
    for (int i = 0; i < m; i++) {
        invalid_sequences = (invalid_sequences + dp[n][i]) % MOD;
    }
    debug(total_sequences, invalid_sequences);
