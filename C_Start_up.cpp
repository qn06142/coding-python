...
int main(){
    ...
    while(true){
        int n;
        if(!(cin >> n)) return 0;
        vector<long long> t(n+1), a(n+1);
        int m = 0;
        for(int i = 1; i <= n; i++) {
            int t_, h_; cin >> t_ >> h_;
            while(m and h_ >= a[m]) --m;
            ++m;
            t[m] = t_;
            a[m] = h_;
        }
        ...
        for(int i = 1; i <= n; i++) {
            while(ind < i and dp[ind] <= t[i]) ++ind;
            dp[i] = (long long) t[i] + (long long) 2 * a[ind];
            while(!dq.empty() and dq.front() < ind) dq.pop_front();
            if(!dq.empty()) {
                dp[i] = min(dp[i], dp[dq.front()] + (long long) 2 * a[dq.front() + 1]);
            }
            while(!dq.empty() and dp[dq.back()] + (long long) 2 * a[dq.back() + 1] >= dp[i] + (long long) 2 * a[i + 1]) dq.pop_back();
            dq.push_back(i);
        }
        cout << dp[n] << '\n';
    }
