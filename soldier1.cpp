...
int main() {
    ...
    vector<vector<bool>> dp_fwd(n + 1, vector<bool>(k + 1, false));
    dp_fwd[0][0] = true;
    ...
    for (int i = 1; i <= n; i++) {
        for (int j = 0; j <= k; j++) {
            ...
            if(j < a[i]) {
                dp_fwd[i][j] = dp_fwd[i - 1][j]; 
            }
            if (j >= a[i]) {
                dp_fwd[i][j] = dp_fwd[i - 1][j] || dp_fwd[i - 1][j - a[i]]; 
            }
        }
    }

    
    vector<vector<bool>> dp_rev(n + 2, vector<bool>(k + 1, false));
    dp_rev[n+1][0] = true;
    ...
    for (int i = n; i >= 1; i--) {
        for (int j = 0; j <= k; j++) {
            ...
            if(j < a[i]) {
                dp_rev[i][j] = dp_rev[i + 1][j];
            }
            if (j >= a[i]) {
                dp_rev[i][j] = dp_rev[i + 1][j] | dp_rev[i + 1][j - a[i]]; 
            }
        }
    }

    
    vector<int> important(n + 1, 0);
    for (int i = 1; i <= n; i++) {
    	if (a[i] >= k) {
    		important[i] = true;
    		continue;
    	...
    	for (int pref = 0; pref < k; pref++) {
    		if (!dp_fwd[i - 1][pref]) continue;
    		// k > pref + suff >= k - a[i]
    		// k - pref > suff >= k - a[i] - pref
    		for (int suff = max(0, k - a[i] - pref); suff < k - pref; suff++) {
    			if (dp_rev[i + 1][suff]) {
    				important[i] = true;
    				goto lmao;
    			}
