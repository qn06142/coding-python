...
def generate_test_case(n = 500000, q = 500000, value_range=(- 10 ** 9, 10 ** 9)):
    m = 2 * n + 1
    a = [random.randint(*value_range) for _ in range(m)]
    queries = []
    for _ in range(q):
        i = random.randint(0, m - 1)
        k = random.randint(*value_range)
        queries.append((i, k))
...
def run_test_case(executable1, executable2, test_case):
    output1 = run_program(executable1, test_case, tle = False)
...
def main():
    executable1 = './lmh_median'
    executable2 = './lmh_median'
