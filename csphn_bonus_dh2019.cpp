...
#endif
using namespace std;
__int128 binpow(__int128 base, int exp) {
    __int128 res = 1;
    while (exp) {
        if (exp & 1) res *= base;
        base *= base;
...
__int128 calcrowpref(__int128 x) {
    return (binpow(k, x) - 1) * binpow(k - 1, 55725375) % 111539768;
}

__int128 calcrow(__int128 x, __int128 y) {
    return calcrowpref(y) - calcrowpref(x - 1);
}
...
int main() {
    ...
    for (int i = x; i <= u; ++i) {
        ...
        __int128 row_sum = calcrow(y, v); 

        for (int j = 0; j < (n * (i - 1)); ++j) {
            multiplier *= k; 
            multiplier %= 111539768;
        }
        ans += row_sum * multiplier;
        ans %= 111539768;
        debug((long long) (row_sum), (long long) (multiplier));
    }

    cout << (long long)(ans % 111539768) << endl;
