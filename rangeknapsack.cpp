...
int main() {
    ...
    int sqrtn = sqrt(n);
    stor.resize(sqrtn + 5, vector<vector<int>>(sqrtn + 5));
    for(int i = 1; i * sqrtn <= n; i++) {
        stor[i][i - 1] = vector<int>(maxw + 5);
    }
    for(int i = 1; i * sqrtn <= n; i++) {
        for(int j = i; j * sqrtn <= n; j++) {
            int l = i * sqrtn;
            int r = j * sqrtn;
            stor[i][j] = stor[i][j - 1];
            if(l == r) {
    ...
    while(q--) {
        ...
        cin >> l >> r >> w1;
        int bl = -1, br = -1;
        for(int i = 1; i * sqrtn <= r; i++) {
            if(i * sqrtn >= l) {
                if(bl == -1) bl = i;
                br = i;
            }
        }
        vector<int> stor1(maxw + 5);
        debug(br, bl);
        if(bl == -1 or br == -1) {
            calc(l, r, stor1);
        } else {
            stor1 = stor[bl][br];
            calc(l, bl * sqrtn - 1, stor1);
            calc(br * sqrtn + 1, r, stor1);
        }
        debug(stor1);
        cout << stor1[w1] << '\n';
    }
