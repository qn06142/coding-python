...
set<pair<pair<int, int>, int>> stor; // {{l, r}, cn\}
int ans;
int POW(int a, int b) {
    if(b == 0) return 1;
    int res = POW(a, b / 2);
...
void add(int l, int r) {
    set<pair<pair<int, int>, int>>::iterator x, y;
    x = stor.lower_bound({{l, -1}, -1});
    y = stor.upper_bound({{r, INT_MAX}, -1});
    if(x != stor.begin()) x--;
    if(x->first.second < l) x++;
    int sum = 1;
    long long tmp = 1;
    for(set<pair<pair<int, int>, int>>::iterator i = x; i != y; i++) {
        sum += i->second;
        tmp *= i->second;
        tmp %= mod;
