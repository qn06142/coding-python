...
int main() {
    ...
    d.reserve(m);
    for (int i = 0; i < m; ++i) d.emplace_back(b[i] - a[i], i);
    sort(d.begin(), d.end(), greater<>()); 

    vector<int> assign(m, 2); 
    for (int k = 0; k < n; ++k) assign[d[k].second] = 1;

    long long total = 0;
    for (int i = 0; i < m; ++i) {
        total += (assign[i] == 1) ? a[i] : b[i];
    }

    cout << total << '\n';

    vector<int> tin1;
    for (int i = 0; i < m; ++i) if (assign[i] == 1) tin1.push_back(i+1);
}