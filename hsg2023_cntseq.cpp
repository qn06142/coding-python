...
void count_(int n, vector<int>& a, int m, vector<pair<long long, long long>>& queries) {
    ...
    for (int i = 0; i < n; ++i) {
        ...
        long long numSubarrays = (i - l + 1) * (r - i + 1); 
        count[a[i]] += numSubarrays;
    }
    ...
    for (int i = 0; i < m; ++i) {
        long long L = queries[i].first;
        long long R = queries[i].second;

        auto itR = pref.lower_bound(R + 1); 
        auto itL = pref.lower_bound(L);    

        long long sumR = (itR == pref.begin()) ? 0 : prev(itR)->second;
        long long sumL = (itL == pref.begin()) ? 0 : prev(itL)->second;
