...
def generate_test_case() -> str:
    n = random.randint(1, 5)          # number of books
    m = random.randint(1, 200)         # budget
    # pick codes from 1..(n//2 + 2) so duplicates are possible
    codes = [random.randint(1, n//2 + 2) for _ in range(n)]
    prices = [random.randint(1, 50) for _ in range(n)]

    out = []
    out.append(f"{n} {m}")
    out.append(" ".join(map(str, codes)))
    out.append(" ".join(map(str, prices)))
    return "\n".join(out)
...
def main():
    executable1 = './lmh_books1'
    executable2 = './lmh_books'
