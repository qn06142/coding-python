...
def generate_test_case():
    # Randomly generate values for n (number of push-ups) and m (number of cards)
    n = 5000
    m = random.randint(1, 10)    # m can be between 1 and 10

    # Randomly generate the card values (each between 1 and 20)
    cards = [random.randint(1, 30) for _ in range(m)]
    
    # Generate the first line: n and m
    first_line = f"{n} {m}"
    
    # Generate the second line: the card values
    second_line = " ".join(map(str, cards))
    
    # Combine both lines into the final test case string
    return f"{first_line}\n{second_line}"
