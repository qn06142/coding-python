...
int pre[1 << 16];
pair<int, int> diff(int mask1, int mask2) {
    int diffm = mask1 ^ mask2;
    bool tmp = 0;
    pair<int, int> ans;
    for(int i = 1; i <= n; i++) {
        debug(i);
...
signed main() {
    ...
    for(int mask = 0; mask < (1 << n); mask++) {
        for(int i = 1; i <= n; i++) {
            for(int j = i + 1; j <= n; j++) {
                if(check(mask, i - 1) or check(mask, j - 1)) continue;
                int curr = c1[i][mask] * r + c1[j][mask] * r + c[i][j] * f;
                int newmask = mask | (1 << (i - 1)) | (1 << (j - 1));
                if(dp[newmask] > dp[mask] + curr) {
                    pre[newmask] = mask;
                }
                dp[newmask] = min(dp[newmask], dp[mask] + curr);
            }
        }
    }
    cout << dp[(1 << n) - 1] << '\n';
    int init = ((1 << n) - 1);
    while(init != 0) {
        pair<int, int> tmp = diff(init, pre[init]);
