...
//    i   tight iszero nodigit[if the bit i is used as the digit in prev]
int dp[20][2]   [2]    [(1 << 10)];
int s[20];
int recur(int i, bool tight, bool iszero, int nodigit) {
    if(i < 0) {
        return 1;
    }
    if(!tight and dp[i][tight][iszero][nodigit] != -1) return dp[i][tight][iszero][nodigit];

    int newlim = (tight ? s[i] : 9);
    int ans = 0;
    for(int c = 0; c <= newlim; c++) {
        if(iszero or c != 0) {
            if(!(nodigit >> c & 1))
            ans += recur(i - 1, tight and c == newlim, iszero or c != 0, nodigit | (1 << c));
        } else {
            ans += recur(i - 1, tight and c == newlim, iszero, nodigit);
        }
    }
    if(!tight) dp[i][tight][iszero][nodigit] = ans;
    return ans;
}
int f(int n) {
    int ind = 0;
