...
int main() {
    ...
    sort(pts.begin(), pts.end(), [](const pair<long long,long long>& a, const pair<long long,long long>& b){
        if (a.first != b.first) return a.first < b.first;
        return a.second < b.second;
    });

    vector<long long> h; h.reserve(n); 

    for (int i = 0; i < n; ++i) {
        long long y = pts[i].second;
        auto it = lower_bound(h.begin(), h.end(), y, [](long long a, long long b){
            return a > b; 
        });
        if (it == h.end()) h.push_back(y);
        else *it = y;
    }
