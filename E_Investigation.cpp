...
int dp[11][90][90], num[11], l, r, k, cnt, T;
int dfs(int p, int lim, int mo, int mod) {
	if(!p) return !mo && !mod;
	if(!lim && ~dp[p][mo][mod]) return dp[p][mo][mod];
	int maxn = lim ? num[p] : 9, res = 0;
	for(int i = 0; i <= maxn; i++) res += dfs(p - 1, lim && i == maxn, (mo + i) % k, (mod * 10 + i) % k);
	return lim ? res : dp[p][mo][mod] = res;
}
int get(int k) {
	for(cnt = 0; k; k /= 10) num[++cnt] = k % 10;
