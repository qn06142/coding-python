...
int main() {
    ...
    for(int i = 0; i <= n; i++) {
        for(int j = 0; j <= w; j++) {
            for(int k = 0; k <= w; k++) {
                if(j + a[i + 1] <= w) {
                    dp[i + 1][j + a[i + 1]][k] = min(dp[i][j][k], dp[i + 1][j + a[i + 1]][k]);
                }
                if(k + a[i + 1] <= w) {
                    dp[i + 1][j][k + a[i + 1]] = min(dp[i][j][k], dp[i + 1][j][k + a[i + 1]]);
                }
                dp[i + 1][a[i + 1]][k] = min(dp[i][j][k] + 1, dp[i + 1][a[i + 1]][k]);
                dp[i + 1][j][a[i + 1]] = min(dp[i][j][k] + 1, dp[i + 1][j][a[i + 1]]);
            }
        }
    }
    int ans = 1e9;
