...
int calculatePerimeter(const vector<int>& heights) {
    int perimeter = 0;
    int n = heights.size();
    ...
    for (int i = 0; i < n; ++i) {
        if (i == 0) {
            ...
            perimeter += abs(heights[i] - heights[i - 1]);
        }
    }

    perimeter += n * 2 + heights[n - 1];
    return perimeter;
}
...
int main() {
    ...
    while (cin >> N && N != 0) {
        ...
        vector<int> perm = heights;
        int maxPerimeter = 0;
        int count = 0;

        sort(perm.begin(), perm.end());
        do {
            int currentPerimeter = calculatePerimeter(perm);
            if (currentPerimeter > maxPerimeter) {
                maxPerimeter = currentPerimeter;
                count = 1;
            } else if (currentPerimeter == maxPerimeter) {
                ++count;
            }
            debug(perm, currentPerimeter);
        } while (next_permutation(perm.begin(), perm.end()));

        cout << maxPerimeter << " " << count << endl;
    }
