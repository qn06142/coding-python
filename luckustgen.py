...
def generate_test_case():
    n = 10  # Small n for the testcase
    m = random.randint(10, 100)  # Maximum weight capacity of the bag

    # Generate n items with random weights and values
    items = [(random.randint(1, 20), random.randint(1, 100)) for _ in range(n)]

    q = random.randint(1, 5)  # Number of plans

    # Generate q plans with random valid ranges
    plans = [
        (random.randint(1, n), random.randint(1, n))
        for _ in range(q)
    ]

    # Ensure valid ranges for plans
    plans = [(min(l, r), max(l, r)) for l, r in plans]

    # Create the testcase as a string
    testcase = f"{n} {m}\n"
    testcase += "\n".join(f"{w} {v}" for w, v in items) + "\n"
    testcase += f"{q}\n"
    testcase += "\n".join(f"{l} {r}" for l, r in plans)
