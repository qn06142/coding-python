...
int main() {
    ...
    vector<vector<long long>> cost_i(n, vector<long long>(FULL, 0));
    for (int i = 0; i < n; ++i) {
        for (int mask = 1; mask < FULL; ++mask) {
            int b = __builtin_ctz(mask);
            int prev = mask ^ (1<<b);
            cost_i[i][mask] = cost_i[i][prev] + cost[i][b];
        }
    }
    ...
    for (int mask = 0; mask < FULL; ++mask) {
        ...
        for (int sub = rem; sub; sub = (sub - 1) & rem) {
            if (__builtin_popcount((unsigned)sub) != gs) continue;
            int newmask = mask | sub;

            long long newc = 0;
            for (int i_mask = mask; i_mask; i_mask &= (i_mask - 1)) {
                int i = __builtin_ctz(i_mask);
                newc += cost_i[i][sub];
            }

            dp[newmask] = min(dp[newmask], dp[mask] + newc);
        }
    }
