...
bool check(int x, int y, int z, int t) {
    for(int i = x; i <= z; i++) {
        for(int j = y; j <= t; j++) {
            bool above = (j == y ? 1 : mat[i][j] ^ mat[i][j - 1]);
            bool left = (i == x ? 1 : mat[i][j] ^ mat[i - 1][j]);
            if(!above or !left) return false;
        }
    }
    return true;
}
int calcarea(int x, int y, int z, int t) {
    return 1LL * (t - y + 1) * (z - x + 1);
}
int main () {
    int n, m;
    cin >> n >> m;
    ...
    for(int i = 1; i <= n; i++) {
        for(int j = 1; j <= m; j++) {
            for(int a = i; a <= n; a++) {
                for(int b = j; b <= m; b++) {
                    if(check(i, j, a, b)) ans = max(ans, calcarea(i, j, a, b));
                }
            }
        }
    }
