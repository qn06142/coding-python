...
void dfs(long long current_power, bool used_halve) {
    if (current_power > T || visited[current_power][used_halve]) return;

    visited[current_power][used_halve] = true;

    max_power = max(max_power, current_power);

    dfs(current_power + A, used_halve);
    dfs(current_power + B, used_halve);

    if (!used_halve) dfs(current_power / 2, true);
}

int main() {
    cin >> T >> A >> B;

    visited.assign(T + 1, vector<bool>(2, false));
