...
int digit_dp(int pos, int diff, bool is_tight, bool has_started) {
    if (pos == digits.size()) {
        return is_prime[abs(diff)]; 
    }
    if (dp[pos][diff + 50][is_tight][has_started] != -1) {
        return dp[pos][diff + 50][is_tight][has_started];
    }
    ...
    for (int d = 0; d <= limit; d++) {
        int new_diff = diff;
        if (has_started) {
            if ((digits.size() - pos) % 2 == 0) {
                new_diff += d; 
            } else {
                new_diff -= d; 
            }
        }
        ans += digit_dp(pos + 1, new_diff, is_tight && (d == limit), has_started || (d > 0));
    }

    return dp[pos][diff + 50][is_tight][has_started] = ans;
}
...
int count_lucifer(int n) {
    ...
    return digit_dp(0, 0, true, false);
}
