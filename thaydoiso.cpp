...
const int MAXN = 2005;
int a[MAXN], gcd_left[MAXN], gcd_right[MAXN];
int stor[MAXN][MAXN];
int gcd(int x, int y) {
...
int main() {
    ...
    gcd_left[0] = gcd_right[n + 1] = 0;
    for (int i = 1; i <= n; ++i) gcd_left[i] = gcd(gcd_left[i - 1], a[i]);
    for (int i = n; i >= 1; --i) gcd_right[i] = gcd(gcd_right[i + 1], a[i]);
    for(int i = 1; i <= n; i++) {
        int ans = 0;
        for(int j = i; j <= n; j++) {
            ans = __gcd(a[i], ans);
    ...
    for (int i = 1; i <= n; ++i) {
        for(int j = i; j <= n; j++) {
            int g = __gcd(gcd_left[i - 1], gcd_right[j + 1]);
            g = __gcd(g, stor[i + 1][j - 1]);
