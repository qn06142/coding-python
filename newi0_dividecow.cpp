...
void dfs(int idx, long long sum1) {
    if (idx == n) {
        if (sum1 == target) {
            found = true;
            for (int i = 0; i < n; ++i) {
    ...
    long long rem = (idx+1 <= n-1) ? rems[idx+1] : 0;

    if (sum1 + a[idx] <= target && (sum1 + a[idx] + rem) >= target) {
        assignm[idx] = 1;
        dfs(idx+1, sum1 + a[idx]);
    }

    if (sum1 <= target && (sum1 + rem) >= target) {
        assignm[idx] = 2;
        dfs(idx+1, sum1);
    }
}
