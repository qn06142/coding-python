...
int main() {
    ...
    for(int i = 1; i <= k; i++){
        ...
        for(int j = 0; j <= n; j++) {
            pref[j] = add(dp[(i + 1) & 1][j], (j >= i ? (pref[j - i]) : (0)));
        }
        for(int j = 1; j <= n; j++) {/*
            for(int a = x; a <= n; a++) {
                if(j >= a * i) {
                    dp[i & 1][j] = add(dp[i & 1][j], dp[(i + 1) & 1][j - a * i]);
                }
            }*/
            int maxa = j / i;
            int mina= x;
            if(mina <= maxa)
            dp[i & 1][j] = (pref[j - x * i] - (j - (maxa + 1) * i >= 0 ? pref[(j - (maxa + 1) * i)] : 0) + mod) % mod;
        }
    }
    int ans = 0;
    for(int c = 1; c <= n; c++) {
        if(n >= c * k)
        ans = add(ans, dp[(k + 1) & 1][n - c * k]);
    }
    cout << ans;
    return 0;
}
