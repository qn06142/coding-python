...
int hashp1[95], hashp2[95];
int hasht1[200005][95], hasht2[200005][95];
int powmod1[200005], powmod2[200005];

int calc1(int l, int r, int k) {
    return ((long long) hasht1[r][k] - (long long) hasht1[l - 1][k] * powmod1[r - l + 1] % mod1 + mod1) % mod1;
}

int calc2(int l, int r, int k) {
    return ((long long) hasht2[r][k] - (long long) hasht2[l - 1][k] * powmod2[r - l + 1] % mod2 + mod2) % mod2;
}
...
int main() {
    ...
    for (int i = 1; i <= m; i++) {
        for (int k = 0; k <= 93; k++) {
            hashp1[k] = (hashp1[k] * base1 + (k == normchar(p[i]))) % mod1;
            hashp2[k] = (hashp2[k] * base2 + (k == normchar(p[i]))) % mod2;
        }
    }

    for (int i = 1; i <= n; i++) {
        for (int k = 0; k <= 93; k++) {
            hasht1[i][k] = (hasht1[i - 1][k] * base1 + (k == normchar(t[i]))) % mod1;
            hasht2[i][k] = (hasht2[i - 1][k] * base2 + (k == normchar(t[i]))) % mod2;
        }
    }

    set<pair<int, int>> cnt;
    for (int k = 0; k <= 93; k++) {
        cnt.insert({hashp1[k], hashp2[k]});
    }
    ...
    for (int i = 1; i <= n - m + 1; i++) {
        bool match = true;
        for (int k = 0; k <= 93; k++) {
            pair<int, int> cur_hash = {calc1(i, i + m - 1, k), calc2(i, i + m - 1, k)};
            if (cnt.count(cur_hash) == 0) {
                match = false;
                break;
            }
        }
