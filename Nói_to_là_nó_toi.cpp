...
long long dp[20][11][2][2][2];
int a[20];
int n;
long long recur(int i, int d, bool ld, bool sm, bool lz) {
    if(i >= n + 1) {
        return !ld;
    }
    if(dp[i][d][ld][sm][lz] != -1) {
        debug(i, d, ld, sm);
        return dp[i][d][ld][sm][lz];
    }

    long long ans = 0;
    for(int nd = 0; nd <= (sm ? 9 : a[i]); nd++) {
        long long m = recur(i + 1, nd, ld | (nd == d and lz), sm | (nd < a[i]), lz | (nd != 0));
        ans += m;
    }
    debug(i, d, ld, sm, ans);
    return dp[i][d][ld][sm][lz] = ans;
}
...
signed main() {
    ...
    long long tmp = f("2");
    debug(tmp);
    cout << f(to_string(r)) - ((l > 0) ? f(to_string(l - 1)) : 0);
}