...
bool check(const string &pat, int x) {
    ...
    for (int i = n-1; i >= 0; --i) {
        char dig = char('0' + (x % 10));
        x /= 10;
        if (pat[i] != '?' && pat[i] != dig) return false;
        if (i == 0 && n > 1 && dig == '0') return false; 
    }
    return true;
}
...
int main() {
    ...
    sieve[0] = sieve[1] = 0;
    for (int p = 2; (long long)p * p <= limit; ++p) if (sieve[p]) {
        for (int k = p * p; k <= limit; k += p) sieve[k] = 0;
    }

    long long ans = 0;
    for (int x = low; x <= high; ++x) if (sieve[x]) {
        if (check(s, x)) ++ans;
    }
    cout << ans;
}