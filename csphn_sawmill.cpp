...
long long cost(int i, int j) {
    return pd[i - 1] * pref[i - 1] - pdw[i - 1]
        +  pd[j - 1] * (pref[j - 1] - pref[i]) - (pdw[j - 1] - pdw[i])
        + pd[n] * (pref[n] - pref[j]) - (pdw[n] - pdw[j]);
}
...
void comp(int l, int r, int optl, int optr) {
    ...
    res = min(res, ans);
    comp(l, mid - 1, optl, opt);
    comp(mid + 1, r, opt, optr);
}
int main() {
    cin >> n;
    for(int i = 1; i <= n; i++) {
