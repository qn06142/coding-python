...
const int maxn = 405;
int a[maxn][maxn], pref[maxn][maxn][maxn], col[maxn][maxn];
...
int main() {
    ...
    for(int i = 1; i <= n; i++) {
        for(int j = i; j <= n; j++) {
            for(int k = 1; k <= m; k++) {
                pref[i][j][k] = pref[i][j][k - 1] + (col[j][k] - col[i - 1][k]);
            }
        }
    }
    ...
    for(int a = 1; a <= n; a++) {
        for(int b = a; b <= n; b++) {
            for(int x = 1; x <= m; x++) {
                int l = x, r = m, mid, tmp = -1;
                while(l <= r) {
                    mid = (l + r) / 2;
                    int sum = pref[a][b][mid] - pref[a][b][x - 1];
                    if(sum == 0) {
                        tmp = mid;
                        l = mid + 1;
                    } else r = mid - 1;
