...
long long ans[maxn];
long long dijktra(int u, int v) {
    unordered_set<int> modified;
    priority_queue<pair<int, int>, vector<pair<int, int>>, greater<pair<int, int>>> pq;
    pq.push(make_pair(0, u)); // first is weight, second is ndoe
    ...
    while(!pq.empty()) {
        ...
        for(auto i:adj[u]) {
            ...
            debug(u, v, w);
            debug(ans[v], ans[u]);
            if(ans[v] > max(ans[u], (long long)w)) {
                ans[v] = max(ans[u], (long long)w);
    ...
    debug(modified);
cleanup:
    long long ans_ = ans[v];
    for(int i :modified) {
        ans[i] = LLONG_MAX;
    }
    return (ans_ == LLONG_MAX ? -1 : ans_);
}
signed main() {
    ios_base::sync_with_stdio(false);
    cin.tie(0);
    cout.tie(0);
    freopen("routing.inp", "r", stdin);
