...
int recur(int i, bool tight, bool validzero, int count, int digit) {
    if (i < 0) return count; 

    if (!tight and dp[i][tight][validzero][count] != -1) return dp[i][tight][validzero][count]; 

    int res = 0;
    int limit = tight ? a[i] : 9;

    for (int d = 0; d <= limit; d++) {
        int newcount = count;
        if ((d > 0 || validzero) and (d == digit)) newcount++;
        res = res + recur(i - 1, tight && (d == limit), validzero || (d > 0), newcount, digit);
    }
