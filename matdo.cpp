...
int longestpossum(const vector<int>& arr) {
    int n = arr.size();
    vector<int> pref(n + 1, 0);
    vector<int> minpref(n + 1, 0);
...
int main() {
    ...
    for (char target = 'a'; target <= 'z'; ++target) {
        ...
        for (int i = 0; i < S.size(); ++i) {
            if (S[i] == target) arr[i] = 1;
        }
        debug(arr, longestpossum(arr));
        ans = max(ans, longestpossum(arr));
    }
