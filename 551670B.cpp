...
bool can_find(int z) {
    // Case 1: Try to form subsequence by taking odd-indexed elements (1-based) <= z
    int odd_count = 0;
    for (int i = 0; i < n; ++i) {
        if (odd_count % 2 == 0 && a[i] <= z) {
            odd_count++;
    ...
    // Case 2: Try to form subsequence by taking even-indexed elements (1-based) <= z
    int even_count = 0;
    for (int i = 0; i < n; ++i) {
        if (even_count % 2 == 1 && a[i] <= z) {
            even_count++;
        } else if (even_count % 2 == 0) {
            even_count++;
        }
        if (even_count == k) return true;
    }
