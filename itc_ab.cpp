...
//    i              tight iszero lasta lastb remain
int dp[(int) 1e4 + 5][2]   [2]    [10]  [10]  [2];
int s[(int) 1e4 + 5], ind = 0;
int recur(int i, bool tight, bool iszero, int lasta, int lastb, bool remain) {
    if(i > ind) return (!(remain));
    if(dp[i][tight][iszero][lasta][lastb][remain] != -1) return dp[i][tight][iszero][lasta][lastb][remain];
    int newlim = (tight ? s[i] : 9);
    int ans = 0;
    for(int c = 0; c <= newlim; c++) {
        if(c != lasta and s[i] - c != lastb) {
            ans += recur()
        }
    }
}
