...
int up[maxn][LOG], dep[maxn], gcdup[maxn][LOG];
...
void build() {
    for (int j = 1; j < LOG; j++) {
        for (int u = 1; u <= n; u++) {
            int mid = up[u][j - 1];
            up[u][j] = up[mid][j - 1];
            gcdup[u][j] = __gcd(gcdup[u][j - 1], gcdup[mid][j - 1]);
        }
    }
}
...
#define readChar() (buf_pos == buf_len ? (buf_pos = 0, buf_len = fread(buf, 1, BUFSZ, stdin), buf_len == 0 ? EOF : buf[buf_pos++]) : buf[buf_pos++])
...
int getgcd(int x, int anc) {
    int res = 0;
    int diff = dep[x] - dep[anc];
    for (int i = LOG - 1; i >= 0; i--) {
        if ((diff >> i) & 1) {
            res = __gcd(res, gcdup[x][i]);
            if (res == 1) return 1;
            x = up[x][i];
        }
    }
