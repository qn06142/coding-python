...
int main(){
    ...
    vector<pair<long long,int>> b(m+1);
    for(int j = 1; j <= m; j++){
    ...
    for(int j = 1; j <= m; j++){
        long long pos = b[j].first;
        int cap = b[j].second;
        pref[0] = 0;
        for(int i = 1; i <= n; i++){
            pref[i] = pref[i-1] + llabs(a[i] - pos);
        }
        deque<int> dq;
        dq.push_back(0);

        for(int k = 0; k <= n; k++){
            while(!dq.empty() && dq.front() < k - cap) dq.pop_front();
            dp[j][k] = dp[j-1][dq.front()] - pref[dq.front()] + pref[k];
            if(k < n){
                long long tmp = dp[j-1][k] - pref[k];
                while(!dq.empty() && (dp[j-1][dq.back()] - pref[dq.back()]) >= tmp)
                    dq.pop_back();
                dq.push_back(k);
            }
        }
