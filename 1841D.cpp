...
int main() {
    ...
    while(q--) {
        ...
        for(int i = 1; i <= n; i++) {
            for(int j = i + 1; j <= n; j++) {
                if(max(lines[i].first, lines[j].first) <= min(lines[i].second, lines[j].second)) {
                    stor.push_back(make_pair(min(lines[i].first, lines[j].first), max(lines[i].second, lines[j].second)));
                }
            }
        }
