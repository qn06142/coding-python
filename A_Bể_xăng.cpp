...
signed main() {
    ...
    stack<int> stacc;
    for(int i = 1; i <= n; i++) {
        while(!stacc.empty() && h[stacc.top()] < h[i]) stacc.pop();
        l[i] = stacc.empty() ? 0 : stacc.top();
        stacc.push(i);
    }
    ...
    for(int i = n; i >= 1; i--) {
        while(!stacc.empty() && h[stacc.top()] < h[i]) stacc.pop();
        r[i] = stacc.empty() ? n + 1 : stacc.top();
        stacc.push(i);
    }

    for(int i = 1; i <= n; i++) {
        int j = l[i];
        dp1[i] = dp1[j] + (v[i] - v[j] - 1) * h[i] - sum(j + 1, i - 1);
    }

    for(int i = n; i >= 1; i--) {
        int j = r[i];
        dp2[i] = dp2[j] + (v[j] - v[i] - 1) * h[i] - sum(i + 1, j - 1);
    }
    debug(dp1);
    debug(dp2);
    int q;
    ...
    while(q--) {
        int k;
        cin >> k;
        int it1 = lower_bound(dp1, dp1 + n + 1, k) - dp1 - 1;
        int it2 = n - (upper_bound(dp2 + 1, dp2 + n + 2, k,  greater<int>()) - dp2 - 1);
        debug(it1, it2);
        cout << min(n, it1 + it2) << '\n';
    }
