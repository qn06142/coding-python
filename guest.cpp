...
int par[(int) 1e5 + 5];
int val[(int) 1e5 + 5];
vector<int> childs[(int) 1e5 + 5];
void dfs(int p, int pv) {
    int sum1 = 0, sum2 = 0;
    for (int i : childs[p]) {
        if (i == pv) continue;
        dfs(i, p);
        sum1 += dp[i][1]; // Include child in the set
        sum2 += max(dp[i][0], dp[i][1]); // Exclude child from the set
    }
    dp[p][0] = val[p] + sum1; // Include current node
    dp[p][1] = sum2;          // Exclude current node
}
...
signed main() {
    ...
    for(int i = 1; i <= n; i++) {
        ...
        par[i] = b;
        val[i] = h;
        childs[b].push_back(i);
    }
    dfs(parent, 0);
    cout << max(dp[parent][0], dp[parent][1]);
}