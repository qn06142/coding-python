...
int main() {
    ...
    const int HALF = 8;
    const int LIM = 1 << HALF; 

    for (int i = 0; i < LIM; ++i) pop8[i] = (unsigned char)__builtin_popcount(i);
    ...
    for (int idx = 0; idx < n; ++idx) {
        ...
        static int ans[17];
        int maxK = m;
        for (int t = 0; t <= maxK; ++t) ans[t] = 0;

        for (int prev_ = 0; prev_ < LIM; ++prev_) {
            int k_low = pop8[low ^ prev_]; 

            const int *row = cnt[prev_][high];
        ...
        for (int suf_ = 0; suf_ < LIM; ++suf_) {
            int k_high = pop8[high ^ suf_]; 
            ++cnt[low][suf_][k_high];
        }
    }
}