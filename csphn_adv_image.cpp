...
bool issolid(int x, int y, int a, int b) {
    int c = cnt(x, y, a, b);
    int area = (a - x + 1) * (b - y + 1);
    return c == 0 || c == area;
}

string recur(int x, int y, int a, int b) {
    if (issolid(x, y, a, b)) {
        return (mat[x][y] ? "00" : "01");
    }

    int midx = (x + a) / 2;
    int midy = (y + b) / 2;
    string A = recur(x, y, midx, midy);             
    string B = recur(x, midy+1, midx, b);           
    string C = recur(midx+1, y, a, midy);           
    string D = recur(midx+1, midy+1, a, b);         
    return "1" + A + B + C + D;
}
