...
int main() {
    ...
    for (int i = 1; i < (1 << m); ++i) {
        long long lcm = 1;
        int bits = __builtin_popcount(i); //amount of chosen elements
        for (int j = 0; j < m; ++j) {
            if (i & (1 << j)) { 
                lcm = std::lcm(lcm, (long long)divisors[j]);
            }
        }
        if (lcm > N) continue;

        if (bits % 2 == 1) { //the pattern of even and odd deletion carries pretty easily :D
            count += N / lcm;
        } else {
            count -= N / lcm;
        }
