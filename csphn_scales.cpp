...
void generateSums(int start, int end, vector<int> &sums, int currentSum = 0) {
    if (start > end) {
        sums.push_back(currentSum);
        return;
    }
    generateSums(start + 1, end, sums, currentSum);                  
    generateSums(start + 1, end, sums, currentSum + arr[start]);     
}
...
int main() {
    ...
    if (n <= 21) {
        ...
        generateSums(1, 21, sumsA);    
        generateSums(22, n, sumsB);    
        ...
        for (int sumA : sumsA) {
            if (sumA <= c) {
                ...
                auto it = upper_bound(sumsB.begin(), sumsB.end(), c - sumA);
                if (it != sumsB.begin()) {
                    bestSum = max(bestSum, sumA + *(--it));
                }
            }
        }
        cout << bestSum;
