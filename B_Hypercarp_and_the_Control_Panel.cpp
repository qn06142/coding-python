...
int main() {
    ...
    while(t--) {
        ...
        for(int i = 1; i < blocks.size() - 1; i++) {
            if((blocks[i - 1].second >= 2 or blocks[i + 1].second >= 2)and blocks[i].second == 1 and blocks[i - 1].first != blocks[i + 1].first) {
                maxc = 1;
            }
        }
        for(int i = 1; i < blocks.size(); i++) {
            if(blocks[i - 1].second >= 2 and blocks[i].second >= 2) {
                maxc = 2;
            }
        }
        debug(maxc, t, blocks);
        cout << blocks.size() + maxc - 2 << '\n';
    }
}