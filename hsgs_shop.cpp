...
void dfs(long long score, long long weight) {

    if (timed_out ||
        chrono::duration_cast<chrono::milliseconds>(Clock::now() - start_time).count() >= 900)
    {
        timed_out = true;
        return;
    }
    ...
    for (int i = 0; i < n; i++) {
        ...
        bool ok = true;
        for (int pre : comp[i]) {
            if (!used[pre]) { ok = false; break; }
        }
        if (!ok) continue;

        if (weight + a[i].first > m) continue;

        used.set(i);
        curr_path.push_back(i);
        dfs(score + a[i].second, weight + a[i].first);
        curr_path.pop_back();
        used.reset(i);
