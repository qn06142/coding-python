...
int main(){
    ...
    ll total_c = 0;
    for(int x : a) if(x == c) total_c++;

    const int Amax = 500000;
    static ll gain[Amax+1];
    fill(gain, gain + Amax + 1, LLONG_MIN/2);
    ll off = 0;
    ll ans = 0;  
    ...
    for(int x : a) {
        if(x == c) {
            ...
            gain[x] = max(gain[x], -off) + 1;

            ans = max(ans, gain[x] + off);
        }
    }

    cout << total_c + max<ll>(0, ans) << "\n";
}