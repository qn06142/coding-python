...
int main(){
    ...
    vector<long long> l_(n+1), r_(n+1);
    ...
    st.reserve(n);
    for(int i=1;i<=n;i++){
        while(!st.empty() && x[st.back()] >= l_[i]){
            int j = st.back(); st.pop_back();

            if(l_[j] < l_[i]) l_[i] = l_[j];
            if(r_[j] > r_[i]) r_[i] = r_[j];
            if(lb[j] < lb[i]) lb[i] = lb[j];
            if(rb[j] > rb[i]) rb[i] = rb[j];
        }
        st.push_back(i);
    }
    ...
    __int128 ans = 0;
    for(int i=1;i<=n;i++){
        long long s = (long long)(rb[i] - lb[i] + 1);
        ans += (__int128)i * (__int128)s;
    }
