...
struct ConvexHullTrick{
    ...
    long double slope(Lines x, Lines y){
        return (long double) (y.b - x.b) / (long double) (x.a - y.a);
    }

    bool better(Lines pre, Lines cur, Lines new_line){
        return slope(pre, new_line) <= slope(cur, new_line);
    }

    void Add(Lines new_line){
        while((int) dq.size() >= 2 && better(new_line, dq.back(), dq[(int) dq.size() - 2])) dq.pop_back();
        dq.push_back(new_line);
    }

    long long Get(long long x){
        while((int) dq.size() >= 2 && dq[0].f(x) >= dq[1].f(x)) dq.pop_front();
        return dq[0].f(x);
    }
} CHT;
int main() {
    ...
    vector<pair<long long,long long>> uniq;
    uniq.reserve(N);
    for (auto &p : v) {
    ...
    for (int i = 1; i <= m; ++i) {
        ...
        ConvexHullTrick::Lines ln;
        ln.a = rects[i-1].second; 
        ln.b = dp[i-1];
        cht.Add(ln);

        long long x = rects[i-1].first;
        dp[i] = cht.Get(x);
    }
