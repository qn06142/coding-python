...
signed main() {
    ...
    int ans = 0;
    for(int i = 1; i <= n; i++) {
        stor.push_back({a[i], i});
        for(pair<int, int> & j : stor) {
            j.first = __gcd(a[i], j.first);
        }
        stor.resize(unique(stor.begin(), stor.end(), [&](pair<int, int> a, pair<int, int> b) {return a.first == b.first;}) - stor.begin());
        debug(stor);
        for(pair<int, int> & j : stor) {
            if(i - j.second + 1 >= k)
            ans = max(ans, (pref[i] - pref[j.second - 1]) * j.first);
        }
    }
    cout << ans;
}