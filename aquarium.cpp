...
signed main() {
    ...
    priority_queue<pair<int, int>, vector<pair<int, int>>, function<bool(pair<int, int>, pair<int, int>)>> stor(comp);
    for(int i = 1; i <= m; i++) {
        stor.push({0, 0});
    }
    for(int i = 1; i <= n; i++) {
        pair<int, int> cur = stor.top();
        stor.pop();
        cur = {cur.first + a[i], cur.second + 1};
        stor.push(cur);
    }
    debug(stor);
    int ans = 0;
    while(!stor.empty()) {
        ans += stor.top().first * stor.top().second;
        stor.pop();
    }
    cout << ans;
    return 0;
