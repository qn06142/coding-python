...
int main() {
    ...
    vector< priority_queue<long long, vector<long long>, greater<long long>> > city(m+1);

    priority_queue<pair<long long,int>> global; 

    vector<long long> output; output.reserve(n);

    for (int i = 0; i < n; ++i) {
        int type; cin >> type;
        debug(type);
        if (type == 1) {
            int T; long long W; cin >> T >> W;

            city[T].push(W);

            long long cur_min = city[T].top();
            global.emplace(cur_min, T);
        } else if (type == 2) {
            ...
            while (true) {
                ...
                if (!city[t].empty() && city[t].top() == val) {

                    city[t].pop();

                    if (!city[t].empty()) {
                        global.emplace(city[t].top(), t);
                    }
