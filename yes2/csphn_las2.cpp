...
int main(){
    ...
    for(int i = 0; i < n; i++){
        for(int j = 0; j < i; j++){
            int d = a[i] - a[j];
            auto &dj = diffs[j];
            int id_j = int(lower_bound(dj.begin(), dj.end(), d) - dj.begin());
            int len  = 2;
            if (id_j < (int)dj.size() && dj[id_j] == d) {
                len = dp[j][id_j] + 1;
            }
            auto &di = diffs[i];
            int id_i = int(lower_bound(di.begin(), di.end(), d) - di.begin());
            dp[i][id_i] = max(dp[i][id_i], len);
            best        = max(best, dp[i][id_i]);
        }
    }
