...
long long recur(int pos, bool is_tight, int first_digit, bool leading_zero, int last_digit, const vector<int>& digits, string deb = "") {
    if (pos == -1) {
        if(last_digit == first_digit)
        debug(deb, last_digit, first_digit);
        return last_digit == first_digit and leading_zero;
    }
    if (dp[pos][is_tight][first_digit][leading_zero][last_digit] != -1) {
        return dp[pos][is_tight][first_digit][leading_zero][last_digit];
    }
    long long count = 0;
    int limit = is_tight ? digits[pos] : 9;

    for (int d = 0; d <= limit; ++d) {
        bool new_tight = is_tight && (d == digits[pos]);
        if (!leading_zero and d != 0) {
            count += recur(pos - 1, new_tight, d, true, d, digits, deb + to_string(d));
        } else 
        count += recur(pos - 1, new_tight, first_digit, leading_zero or d != 0, d, digits, deb + to_string(d));
        
    }

    return dp[pos][is_tight][first_digit][leading_zero][last_digit] = count;
}

long long f(long long n) {
    if (n == 0) return 0;
    vector<int> digits = get(n);
    memset(dp, -1, sizeof(dp));
    return recur(digits.size() - 1, 1, 0, 0, -1, digits);
}
