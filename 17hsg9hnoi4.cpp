...
#define debugArr(...)
#endif
int mindiff(vector<int> &arr, int minLength) {
    int n = arr.size();
    int ans = INT_MAX;
    int windowSum = 0;
...
int getsum(int i, int j, int x, int y) {
    return pref[x][y] - pref[i - 1][y] - pref[x][j - 1] + pref[i - 1][j - 1];
}
int main() {
    cin >> n >> m;
    cin >> r >> d >> h;
    for(int i = 1; i <= n; i++) {
        for(int j = 1; j <= m; j++ ) {
            cin >> mat[i][j];
            pref[i][j] = pref[i - 1][j] + pref[i][j - 1] - pref[i - 1][j - 1] + abs(h - mat[i][j]);
        }
    }
    int ans = INT_MAX;
    for(int i = 1; i <= n; i++) {
        for(int x = i + r - 1; x <= n; x++) {
            vector<int> a;
            for(int k = 1; k <= m; k++) {
                a.push_back(getsum(i, k, x, k));
            }
            ans = min(ans, mindiff(a, d));
            debug(abs(mindiff(a, d)), i, x);
            debug(a);
        }
    }
    cout << ans;
