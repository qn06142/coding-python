...
const int maxn = 5005;
int a[maxn];
long long binpow(long long base, long long exp, long long mod = 998244353) {
    long long result = 1;
    base %= mod;
    while (exp > 0) {
        if (exp % 2 == 1) {
...
int biparte(int a, int b) {
    int ans = 0;
    for(int i = 0; i <= min(a, b); i++) {
        ans += comb(a, i) * comb(b, i) % mod * fact[i] % mod;
        ans %= mod;
    }
    return ans;
}
signed main() {
    ...
    debug(biparte(a, b), biparte(a, c), biparte(b, c));
    cout << biparte(a, b) * biparte(a, c) % mod * biparte(b, c) % mod;
}