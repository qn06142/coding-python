...
int dp[10][2]   [300];
const int delt = 90;
int recur(int i, bool tight, int sum) {
    //debug(i, sum);
    if(i < 0) {
        debug(sum);
        return sum == 1;
    }
    if(dp[i][tight][sum + delt] != -1) return dp[i][tight][sum + delt];
    int newlim = (tight ? a[i] : 9);
    int res = 0;
    dp[i][tight][sum + delt] = 0;
    for(int c = 0; c <= newlim; c++) {
        res += recur(i - 1, tight and (c == a[i]),  ((i % 2) ? 1 : -1) * c + sum);
    }
    return dp[i][tight][sum + delt] = res;
}
int f(int x) {
