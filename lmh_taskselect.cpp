...
map<State, int> dp;
int bacctracc(int rem, int cur, int prev) {
    if (rem == 0) {
        return (cur == target) ? 1 : 0;
    }
    State st = make_tuple(rem, cur, prev);
    if (dp.find(st) != dp.end())
        return dp[st];
    int minad = rem * (prev + 1) + (rem * (rem - 1)) / 2;
    if (cur + minad > target)
        return 0;
    ...
    int end = (target - cur - (rem*(rem - 1)) / 2) / rem;

    if (start > end)
        return 0;

    for (int x = start; x <= end; x++) {
        ways = (ways + bacctracc(rem - 1, cur + x, x)) % MOD;
    }
    dp[st] = ways;
    return ways;
}
