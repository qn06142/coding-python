...
int n;
int coords_x[50000], coords_y[50000];

struct state {
    int x, y, d;
};

struct state* newstate(int x, int y, int d) {
    struct state* a = malloc(sizeof(*a));
    a->x = x;
    a->y = y;
    a->d = d;
    return a;
...
bool reachable(int m) {
    ...
    // mark first m bytes as corrupted
    for (int i = 0; i < m; i++) {
        grid[coords_x[i]][coords_y[i]] = true;
    }
    ...
    q_init(q);
    visited[0][0] = true;
    q_push(q, newstate(0, 0, 0));
    while (!q_empty(q)) {
        struct state* cur = q_front(q);
        q_pop(q);
        ...
        for (int i = 0; i < 4; i++) {
            ...
            if (nx >= 0 && ny >= 0 && nx < MAXN && ny < MAXN 
                ...
                visited[nx][ny] = true;
                q_push(q, newstate(nx, ny, cur->d + 1));
            }
        }
        free(cur);
    }
