...
bool check(int len, int n, int m) {
    ...
    for(int i = 1; i + len - 1 <= n; i++) {
        hashes.insert(gethash(hashs, hashs1, i, i + len - 1));
    }
    for(int i = 1; i + len - 1 <= m; i++) {
        if(hashes.find(gethash(hasht, hasht1, i, i + len - 1))!= hashes.end()) {
            return true;
        }
    }
    return false;
...
int main() {
    #ifndef DEBUG
    freopen("CIRSTR.INP", "r", stdin);
    freopen("CIRSTR.OUT", "w", stdout);
    #endif
    string s, t;
    cin >> s >> t;
    s = s + s;
