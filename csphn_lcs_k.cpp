...
const int N = 1005;
int n, m, k, a[N], b[N], d[N][N];
multiset<int> t[4 * N + 5];
void add(int i, int l, int r, int pos, int v) {
	...
	int mid = l + (r - l) / 2;
	add(i * 2, l, mid, pos, v);
	add(i * 2 + 1, mid + 1, r, pos, v);
	t[i].insert(v);
}
void add(int pos, int v) {
	add(1, 1, N, pos, v);
}
void del(int i, int l, int r, int pos, int v) {
	if (r < pos || l > pos) return;
	if (l == r) {
		t[i].erase(t[i].find(v));
		return;
	}
	int mid = l + (r - l) / 2;
	del(i * 2, l, mid, pos, v);
	del(i * 2 + 1, mid + 1, r, pos, v);
	t[i].erase(t[i].find(v));
}
...
int get(int i, int l, int r, int ql, int qr) {
	...
	if (ql <= l && r <= qr) return t[i].empty()? 0 : *t[i].rbegin();
	int mid = l + (r - l) / 2;
...
signed main() {
    ...
    for (int i = 1; i <= n; i++) {
        ...
        for (int j = 1; j <= m; j++) {
            if (a[i] == b[j]) {
                int maxPrev = get(max(1LL, j - k), j - 1);
                d[i][j] = maxPrev + 1;
                ans = max(ans, d[i][j]);
            }
        }
