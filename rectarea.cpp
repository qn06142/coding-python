...
struct coord {
    ...
    bool operator < (const coord &b) const {
        return tie(a, x, y) < tie(b.a, b.x, b.y);
    }
...
struct event {
    ...
    bool operator<(const event &b) const {
        return tie(x, closed, h) < tie(b.x, b.closed, b.h);
    }
};
coord a[(int) 1e5 + 5];
signed main() {
    ...
    events.reserve(2 * n);
    for(int i = 1; i <= n; i++) {
        events.push_back({a[i].a, 0, a[i].y});
        events.push_back({a[i].x, 1, a[i].y});
    }
    sort(events.begin(), events.end());
    for(event i:events) {
        debug(i.x, i.closed, i.h);
    }
    multiset<int> stor;
    int ans = 0;
    int pre = 0;
    for(int i = 0; i < events.size(); i++) {
        event x = events[i];
        ans += (stor.empty() ? 0 : *stor.rbegin() * (events[i].x - pre));
        debug((stor.empty() ? 0 : *stor.rbegin() * (events[i].x - pre)));
        debug(stor, stor.empty() ? -1 : *stor.rbegin());

        pre = events[i].x;
        if(x.closed) {
            stor.erase(stor.lower_bound(x.h));
        } else {
            stor.insert(x.h);
        }
