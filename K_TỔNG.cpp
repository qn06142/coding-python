...
int dp[MAX_DIGITS][150][2];
vector<int> digits;

int digitDP(int pos, int sum, bool tight, int s) {
    if (sum > s) return 0;  
    if (pos == digits.size()) return sum == s;  
    ...
    for (int d = 0; d <= limit; d++) {
        res += digitDP(pos + 1, sum + d, tight && (d == limit), s);
    }

    if (!tight) dp[pos][sum][0] = res;

    return res;
}

int countNumbersWithSum(int num, int s) {
    digits.clear();
    while (num > 0) {
...
int findSmallest(int a, int b, int s) {
    int low = a, high = b, result = -1;

    while (low <= high) {
        int mid = (low + high) / 2;
        int countInMid = countNumbersWithSum(mid, s);
        int countInA = countNumbersWithSum(a - 1, s);

        if (countInMid - countInA >= 1) {
            result = mid;  
            high = mid - 1;
        } else {
            low = mid + 1;  
...
signed main() {
    ...
    int countInB = countNumbersWithSum(b, s);
    int countInA = countNumbersWithSum(a - 1, s);
    cout << countInB - countInA << endl;

    int result = findSmallest(a, b, s);
    cout << result << endl;
