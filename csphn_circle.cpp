...
int main() {
    ...
    if(n % 3 != 0) {
        ...
        vector<int> a(n + 1);
        while(cnt > 0) {
            a[modind(n, currind, 3)] = b[modind(n, currind, 2)] - b[modind(n, currind, 1)] + a[currind];
            cnt--;
            currind = (currind + 2) % n + 1;
        }
        int min_ = *min_element(a.begin() + 1, a.end());
        for(int i = 1; i <= n; i++) {
            cout << (a[i] = a[i] + 1 - min_) << ' ';
        }
        min_ = *min_element(a.begin() + 1, a.end());
        debug(min_);
        exit(0);
    }
    return 1;
