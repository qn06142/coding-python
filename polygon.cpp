...
int dp[maxn][maxn];
int n;
int prev(int i, int m, int n) { return (i - m + n) % n; }
...
int recur(int l, int r) {
    ...
    int ans = -1e18;
    for(int mid = l; mid != r; mid = prev(mid, 1, n)) {
        int prevm = prev(mid, 1, n);
        char op = a[mid].first;
        int l_ = recur(l, mid);
        int r_ = recur(prevm, r);
        ans = max(ans, calc(op, l_, r_));
    }
    return dp[l][r] = ans;
}
...
signed main() {
    ...
    int ans = -1e18;
    for(int i = 0; i < n; i++) {
        ans = max(ans, recur((i + n - 1) % n, i % n));
    }
    cout << ans << '\n';
    vector<int> tmp;
    for(int i = 0; i < n; i++) {
        if(recur((i + n - 1) % n, i % n) == ans) {
            tmp.push_back(i + 1);
        }
    }
