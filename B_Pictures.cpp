...
int main() {
    ...
    vector<pair<int, int>> paintings(N + 1); 
    ...
    sort(paintings.begin() + 1, paintings.end());
    sort(frames.begin() + 1, frames.end());

    vector<vector<int>> dp(N + 1, vector<int>(M + 1, 0));

    vector<int> prefmax(M + 1, 0);
    ...
    for (int i = 1; i <= N; i++) {
        for (int j = 1; j <= M; j++) {
            ...
            if (paintings[i].second <= frames[j]) {
                dp[i][j] = max(dp[i][j], prefmax[j - 1] + 1);
            }
        }

        for (int j = 1; j <= M; j++) {
            prefmax[j] = max(prefmax[j - 1], dp[i][j]);
        }
    }
    debug(dp);
    int result = 0;
