...
int main() {
    ...
    map<int, vector<int>> stor;
    vector<int> assoc(n + 5, 0);
    SegmentTree seg(n + 5);
    stor[0].push_back(0);
    for (int i = 0; i < n; i++) {
        char c = s[i];
        if (c == '(') {
            assoc[ptr] = ans;
            pref[ptr] = pref[ptr - 1] + 1;
            seg.update(ptr, pref[ptr]);
            stor[pref[ptr]].push_back(ptr);
            ptr++;
        } else if (c == ')') {
            pref[ptr] = pref[ptr - 1] - 1;
            seg.update(ptr, pref[ptr]);
            stor[pref[ptr]].push_back(ptr);
            ptr++;
            auto& tmp = stor[pref[ptr - 1]];
            int validIdx = -1;
            for (int j = 0; j <= tmp.size(); j++) {
                int currPos = tmp[j];
                if (seg.query(currPos + 1, ptr - 1) >= pref[currPos]) {
                    validIdx = j;
                    break;
                }
            }
            debug(validIdx);
            if (validIdx != -1) {
                ans += tmp.size() - validIdx - 1;
            }
            assoc[ptr - 1] = ans;
        } else if (c == '-') {
            ptr--;
            stor[pref[ptr]].pop_back();
            if(stor[pref[ptr]].empty())
            seg.update(ptr, INT_MAX);
            ans = assoc[ptr - 1];
        }
        cout << ans << '\n';
        debug(ptr);
