...
const int M = 2 * 1000000;

vector<long long> calc(vector<int>& xs, int N) {
    vector<long long> xsum(2 * M + 1, 0);
    sort(xs.begin(), xs.end());
    int i = 0;
    xsum[M] = accumulate(xs.begin(), xs.end(), 0LL) + N * 1LL * M;
    for (int x = -M + 1; x <= M; ++x) {
        while (i < N && xs[i] < x) {
            i++;
        }
        xsum[x + M] = xsum[x + M - 1] + 2 * i - N;
    }
    return xsum;
}
...
int main() {
    ...
    vector<long long> xsum = calc(x, N);
    vector<long long> ysum = calc(y, N);
