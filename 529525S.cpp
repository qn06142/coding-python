...
#pragma GCC target("tune=skylake,arch=skylake")
const int maxn = 205;
int dp[maxn][maxn];
string s;
bool check(int l, int r, int l_, int r_) {
    if((r - l) > (r_ -l_)) return false;
    if((r - l) < (r_ - l_)) return true;
    for(int i = 0; i <= r - l; i++) {
        if(s[l + i] < s[l_ + i]) return true;
        if(s[l + i] > s[l_ + i]) return false;
    }
    return false;
}
int main() { 
    ...
    for(int l = n; l >= 1; l--) {
        for(int r = n; r >= l; r--) {
            ...
            for(int l_ = r + 1; l_ <= n; l_++) {
                for(int r_ = l_ + r - l; r_ <= min(l_ + r - l + 1, n); r_++) {
                    if(check(l, r, l_, r_))
                    dp[l][r] = max(dp[l][r], dp[l_][r_] + 1);
                }
            }
        	cerr << l << ' ' << r << " : " << dp[l][r] << '\n';
            ans = max(ans, dp[l][r]);
