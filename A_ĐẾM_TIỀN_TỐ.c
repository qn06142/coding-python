...
struct Node* newnode() {
    ...
    node->c = '\0';
    node->cnt = 0;
    node->nxt = malloc(sizeof(struct Node*) * 26);
    if (!node->nxt) {
        perror("malloc");
        exit(EXIT_FAILURE);
    }
...
// helper to extend a missing child
static void extend_node(struct Node* node, int idx, char c) {
    struct Node* next = newnode();
    next->c = c;
    next->cnt = 1;                // first occurrence of this prefix
    node->nxt[idx] = next;
}

// add a word (string) to the trie
void add_word(struct Node* root, const char* s) {
    struct Node* curr = root;
    curr->cnt++; // sount this word at root as well
    for (int i = 0; s[i]; i++) {
        int idx = s[i] - 'a';
        if (idx < 0 || idx >= 26) continue; // skip non-lowercase

        if (curr->nxt[idx] == NULL) {
            extend_node(curr, idx, s[i]);
        } else {
            curr->nxt[idx]->cnt++;
        }
        curr = curr->nxt[idx];
...
bool search_word(struct Node* root, const char* s) {
    struct Node* curr = root;
    for (int i = 0; s[i]; i++) {
        int idx = s[i] - 'a';
        if (idx < 0 || idx >= 26 || curr->nxt[idx] == NULL)
            return false;
        curr = curr->nxt[idx];
    }
    return curr->cnt > 0; // true if at least one insertion ended here
}

int count_prefix(struct Node* root, const char* prefix) {
    struct Node* curr = root;
...
void free_trie(struct Node* node) {
    if (!node) return;
    for (int i = 0; i < 26; i++) {
        free_trie(node->nxt[i]);
    }
    free(node->nxt);
    free(node);
}
int ans = 0;
void recur(struct Node* prev) {
    ans += (prev-> cnt != 0);
    for(int i = 0; i < 26; i++) {
        if(prev->nxt[i] != NULL) {
            recur(prev->nxt[i]);
