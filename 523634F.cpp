...
vector<pair<int, int>> adj[(int) 1e3 + 5];
vector<int> paths[(int) 1e3 + 5];
vector<int> dijktra() {
    priority_queue<pair<int, int>, vector<pair<int, int>>, greater<pair<int, int>>> pq;
    pq.push({0, 1});
    while(!pq.empty()) {
        pair<int, int> curr = pq.top();pq.pop();
        if(paths[curr.second].size() == k) continue;
        if(!paths[curr.second].empty() and *paths[curr.second].rbegin() >= curr.first) continue;
        paths[curr.second].push_back(curr.first);
        for(pair<int, int> p:adj[curr.second]) {
            pq.push({p.second + curr.first, p.first});
        }
    }
    return paths[n];
}
int main() {
    ...
    vector<int> a = dijktra();
    if(a.size() < k) cout << -1; else
    for(int i:a) cout << i << ' ';
}
