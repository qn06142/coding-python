...
int main() {
    ...
    if (! (p > 0 || q > 0 || r > 0) ) t = 0.0L;

    if (!isfinite(t)) t = 0.0L;

    long double remA = A - p * t;
    long double remB = B - q * t;
    long double remC = C - r * t;
    auto clamp0 = [](long double x) {
        if (fabsl(x) < 1e-12L) return 0.0L;
        return x;
    };
    remA = clamp0(remA);
    remB = clamp0(remB);
    ...
    cout << setprecision(6) << remA << ' ' << remB << ' ' << remC << '\n';
}
