...
int main() {
    ...
    cout.tie(0);
    cin >> n >> m;
    vector<int> l(m + 5), r(m + 5), rm(m + 5), lm(m + 5);
    int ans = 0;
    for(int i = 1; i <= n; i++) {
        for(int j = 1; j <= m; j++) {
            pre[j] = mat[j];
        ...
        for(int _ = 0; _ < 2; _++) {
            stack<int> stacc;
            for(int j = 1; j <= m; j++) {
                while(!stacc.empty() and h[stacc.top()] >= h[j]) stacc.pop();
                l[j] = (stacc.empty() ? 1 : stacc.top() + 1);
                stacc.push(j);
                if(j == 1) {
                    lm[j] = 1;
                } else {
        ...
        reverse(r.begin() + 1, r.begin() + 1 + m);
        reverse(rm.begin() + 1, rm.begin() + 1 + m);
        for(int j = 1; j <= m; j++) {
            int left = max(l[j], lm[j]);
            int right = min(r[j], rm[j]);
            ans = max(ans, (right - left + 1) * h[j]);
        }
        //debug(h);
        //debug(l);
        //debug(r);
