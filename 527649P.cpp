...
int n, m;
int a[(int) 3e5], b[(int) 3e5], bpref[(int) 3e5];
int stor[(int) 3e5], stor1[(int) 3e5], tanks[(int) 3e5];
int f(int x) {
    int l = 1, r = m, ans = -1;
    while(l <= r) {
        int mid = (l + r) / 2;
...
signed main() {
    ...
    for(int i = 1; i <= n; i++) {
        stor[1]++;
        stor[tanks[i]]--;
        stor1[tanks[i]] += a[i] - bpref[tanks[i] - 1];
    }
    for(int i = 1; i<= n; i++) {
        stor[i] += stor[i - 1];
    }
    for(int i = 1; i <= m; i++) {
        if(stor[i] * b[i] + stor1[i] == 0) return 0;
        cout << stor[i] * b[i] + stor1[i] << ' ';
    }
}
