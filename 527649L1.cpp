...
int main() {
    ...
    for(int i = 1; i <= n; i++) {
        ...
        for(int j = i; j <= n; j++) {
            sum += a[j];
            cnt[j].push_back({sum, i});            
        }
    }
    for(int j = 2; j <= n; j++) {
        int sumb = 0;
        sort(cnt[j - 1].begin(), cnt[j - 1].end());
        for(int k = j; k <= n; k++) {
            sumb += a[k];
            int pos = lower_bound(cnt[j - 1].begin(), cnt[j - 1].end(), make_pair(sumb, 0)) - cnt[j - 1].begin();

            if(0 <= pos and pos < cnt[j - 1].size() and cnt[j - 1][pos].first == sumb) {
                ans = max(ans, k - cnt[j - 1][pos].second + 1);
            }
        }
    }
    cout << ans;
