...
__int128 calcrowpref(__int128 x, int k) {
    return (binpow(k, x) - 1) * binpow(k - 1, MOD - 2) % MOD;
}

__int128 calcrow(__int128 x, __int128 y, int k) {
    return (calcrowpref(y, k) - calcrowpref(x - 1, k) + MOD) % MOD;
}
...
int main() {
    ...
    for (int j = y; j <= v; ++j) {
        long long row_sum = 0;
        for (int i = x; i <= u; ++i) {
            __int128 pos = (i - 1) * n + j - 1;
            __int128 cell_val = binpow(k, pos);
            ans = (ans + cell_val) % MOD;
            row_sum += cell_val;
            row_sum %= MOD;
        }
        debug(row_sum);
