...
void backtrackC1(int ind, int diff) {
    if (ind == 0) {
        c1.push_back(diff);
        return;
    }
    backtrackC1(ind - 1, diff - a[ind]);
    backtrackC1(ind - 1, diff + a[ind]);
}

int m;

void backtrackC2(int ind, int diff) {
    if (ind == m) {
        c2.push_back(diff);
        return;
    }
    backtrackC2(ind - 1, diff - a[ind]);
    backtrackC2(ind - 1, diff + a[ind]);
}
...
int main() {
    ...
    for(int i = 0; i < c1.size(); i++) {
        auto it1 = lower_bound(c2.begin(), c2.end(), c1[i]);
        auto it2 = upper_bound(c2.begin(), c2.end(), c1[i]);
        if(it1 != c2.end()) {
            ans = min(ans, abs(c1[i] - *it1));
        }
        if(it2 != c2.end()) {
            ans = min(ans, abs(c1[i] -*it2));
        }
    }
