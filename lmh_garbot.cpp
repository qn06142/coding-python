...
long long calc(long long L, long long R, long long r) {
    if (L > R) swap(L, R);
    if (r <= L)  return R - r;
    if (r >= R)  return r - L;
    long long rf  = 2 * (r - L) + (R - r);
    long long lf = 2 * (R - r) + (r - L);
    return min(rf, lf);
}
...
bool check(long long limit) {
    ...
    for (int i = 1; i <= m && j <= n; ++i) {
        ...
        if (calc(b[j], b[j], r) > limit) continue;

        int lo = j, hi = n, best = j;
        while (lo <= hi) {
            int mid = (lo + hi) / 2;
            if (calc(b[j], b[mid], r) <= limit) {
                best = mid;
                lo = mid + 1;
            } else {
                hi = mid - 1;
