...
int dp[maxn];
int prescol[maxn]; // stores prefixmax for same color
int max1, max2;
int v[maxn];
int c[maxn];
int n;
void solve() {
    ...
    for(int i = 1; i <= n; i++) {
        ...
        // same color
        dp[i] = max(dp[i], prescol[c[i]] + a * v[i]);
        // imposter color
        int tmp;
        if(max1 == prescol[c[i]]) {
            tmp = max2;
        ...
        dp[i] = max(dp[i], tmp + b * v[i]);
        
        if(dp[i] > prescol[c[i]]) {
            if(max1 == prescol[c[i]]) {
                max1 = dp[i];
            } else if(max2 == prescol[c[i]]) {
                max2 = dp[i];
            } else {
                if(dp[i] > max1) {
                    max2 = max1;
        ...
        /*for(int j = 1; j < i; j++) {
            dp[i] = max(dp[i], dp[j] + (c[i] == c[j] ? a * v[i] : b * v[i]));
        }*/
    }
