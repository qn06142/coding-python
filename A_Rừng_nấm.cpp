...
int main() {
    int r, c, d, k;
    cin >> r >> c >> d >> k;
    vector<vector<int>> mat(r + 5, vector<int>(c + 5)), pref(r + 5, vector<int>(c + 5));
    ...
    for(int i = 1; i <= r; i++) {
        for(int j = 1; j <= c; j++) {
            ...
            if(c == '.') continue;
            if(c == 'S') mat[i][j] = 1;
            if(c == 'M') mushrooms.emplace_back(i, j);
        }
    }
    for(int i = 1; i <= r; i++) {
        for(int j = 1; j <= c; j++) {
    ...
    for(pair<int, int> p : mushrooms) {
        ...
        int top_x = max(1, x - d), top_y = max(1, y - d);
        int bottom_x = min(r, x + d), bottom_y = min(c, y + d);

        int total_sprinklers = pref[bottom_x][bottom_y] 
                             - pref[top_x - 1][bottom_y] 
                             - pref[bottom_x][top_y - 1] 
                             + pref[top_x - 1][top_y - 1];
        debug(total_sprinklers);
        ans += total_sprinklers >= k;
    }
    cout << ans;
}