...
#define debugArr(...)
#endif
string make_pal(const string &half, int len) {
    string res = half;
    if (len % 2 == 0) {
        for (int i = half.size() - 1; i >= 0; --i) res += half[i];
    } else {
        for (int i = half.size() - 2; i >= 0; --i) res += half[i];
    }
    return res;
}

string inc_(const string &s) {
    string res = s;
    int carry = 1;
...
string solve(const string &n) {
    int m = n.size();
    int half = (m + 1) / 2;
    string first = n.substr(0, half);

    string pal = make_pal(first, m);
    if (pal > n) return pal;

    string inc = inc_(first);
    if (inc.size() > first.size()) {

        string ans = "1" + string(m - 1, '0') + "1";
        return ans;
    }
    return make_pal(inc, m);
}
