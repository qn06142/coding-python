...
static const int K = 5005;
static const long long INF = 4e18;

int n, k;
static long long c[N][N];
static long long dp[K][N];

void pre(const vector<long long>& x, const vector<long long>& w) {
    static long long sw[N], swx[N];
    for (int i = 1; i <= n; i++) {
        sw[i]  = sw[i-1]  + w[i];
    ...
    for (int l = 1; l <= n; l++) {
        int m = l;
        for (int r = l; r <= n; r++) {
            long long tw = sw[r] - sw[l-1];
            while (m < r && (sw[m] - sw[l-1]) * 2 < tw) m++;
            long long lw  = sw[m-1] - sw[l-1];
            long long lwx = swx[m-1] - swx[l-1];
            long long rw  = sw[r]   - sw[m];
            long long rwx = swx[r]  - swx[m];
            c[l][r] = x[m] * lw - lwx + rwx - x[m] * rw;
        }
    }
}

struct frame { int l, r, ol, orr; };
void solve(int d) {
    static frame st[64];
    int sp = 0;
    st[sp++] = {d, n, d, n};
    while (sp) {
        auto f = st[--sp];
        int l = f.l, r = f.r, ol = f.ol, orr = f.orr;
        if (l > r) continue;
        int mid = (l + r) >> 1;
        long long best = INF; 
        int bk = ol;
        int hi = min(mid, orr);
        for (int j = ol; j <= hi; j++) {
            long long val = dp[d-1][j-1] + c[j][mid];
            if (val < best) {
                best = val;
                bk = j;
            }
