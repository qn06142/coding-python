...
int mat1[maxn][maxn];
int mat2[maxn][maxn];
inline int sum(int x1, int y1, int x2, int y2, int pref[maxn][maxn]) {
...
int main() {
    ...
    debug(mat1);
    debug(mat2);
    int required = (x * y + 1) / 2;
    int ans = 0;
    for(int i = 1; i + x - 1 <= n; i++) {
        for(int j = 1; j + y - 1 <= m; j++) {
            if(sum(i, j, i + x - 1, j + y - 1, mat1) < required and sum(i, j, i + x - 1, j + y - 1, mat2) >= required) {
                ans++;
            }
        }
    }
