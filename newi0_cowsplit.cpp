...
int main() {
    ...
    size_t words = (target + 64) / 64;
    vector<long long> bits(words, 0ULL);
    bits[0] = 1ULL; 
    ...
    for (int idx = 0; idx < n; ++idx) {
        ...
        vector<long long> shifted(words, 0ULL);

        for (size_t i = words; i-- > 0; ) {
            long long src = (long long)i - (long long)word;
            if (src < 0) continue;
            long long val = bits[src];
            long long out = 0ULL;
            if (bit == 0) {
                out = val;
            } else {
        ...
        bits[0] |= shifted[0];
        for (size_t i = 1; i < words; ++i) bits[i] |= shifted[i];

        size_t ind = (size_t)(target / 64);
        int last = (int)(target % 64);
        if (ind < bits.size()) {
            long long mask = (last == 63) ? ~0ULL : ((1ULL << (last + 1)) - 1ULL);
            bits[ind] &= mask;

            for (size_t i = ind + 1; i < bits.size(); ++i) bits[i] = 0ULL;
        }
    }

    long long best = 0;
    for (long long t = target; t >= 0; --t) {
        size_t wi = (size_t)(t / 64);
        int bi = (int)(t % 64);
        if ( (bits[wi] >> bi) & 1ULL ) { best = t; break; }
    }

    long long ans = total - 2 * best;
    cout << ans << '\n';
}