...
signed main() {
    ...
    max1[1] = LLONG_MIN;
    for (int i = 2; i <= n; ++i) {
        max1[i] = max(max1[i - 1], pref[i - 1] * 2);
    }
    debug(max1);
    max2[2] = LLONG_MIN;
    for(int i = 3; i <= n; i++) {
        max2[i] = max(max2[i - 1], max1[i - 1] - 2 * pref[i - 1]);
    }
    debug(max2);
    max3[3] = LLONG_MIN;
    for(int i = 4; i <= n; i++) {
        max3[i] = max(max3[i - 1], max2[i - 1] + 2 * pref[i - 1]);
    }
    /*int ans = LLONG_MIN;
    for (int i = 1; i <= n; ++i) {
        for (int j = i + 1; j <= n; ++j) {
            for (int k = j + 1; k <= n; ++k) {
                int current_res = pref[i - 1] * 2 - 2 * pref[j - 1] + 2 * pref[k - 1] - pref[n];
