...
void sumdiv_avx2() {
    const __m256i v_0_to_7 = _mm256_setr_epi32(0,1,2,3,4,5,6,7);

    for (int i = 1; i < maxn; i++) {
        int stride = i;
        __m256i v_i      = _mm256_set1_epi32(i);
        __m256i v_stride = _mm256_set1_epi32(stride);
        // v_offsets = [0,1,2,3,4,5,6,7] * stride
        __m256i v_offsets = _mm256_mullo_epi32(v_0_to_7, v_stride);

        int j = i;
        for (; j + stride*7 < maxn; j += stride*8) {
            // build indices = j + [0,1,…,7]*stride
            __m256i v_base = _mm256_set1_epi32(j);
            __m256i v_idx  = _mm256_add_epi32(v_base, v_offsets);

            // gather, add, store
            __m256i v_src = _mm256_i32gather_epi32(sumdiv, v_idx, 4);
            __m256i v_res = _mm256_add_epi32(v_src, v_i);

            int out[8];
            _mm256_storeu_si256((__m256i*)out, v_res);

            // scatter using the _actual_ indices from v_idx
            for (int k = 0; k < 8; k++) {
                int dst = _mm256_extract_epi32(v_idx, k);
                sumdiv[dst] = _mm256_extract_epi32(v_res, k);
            }
        }
...
int main() {
    ...
    long long ans = 0;
    for(int i = l; i <= r; i++) {
        long long d = sumdiv[i] - 2LL*i;
        if (d > 0) ans += d; else if (d < 0) ans -= d;
    }
    out1(ans);
}