...
struct CHT {
    ...
    void add(Line l3) {
        while (!dq.empty() && dq.back().m == l3.m) {
            if (l3.c >= dq.back().c) return;
            dq.pop_back();
        }
        while (dq.size() >= 2) {
            Line l1 = dq[dq.size() - 2];
            Line l2 = dq[dq.size() - 1];
            __int128 num1 = l2.c - l1.c;
            __int128 den1 = l1.m - l2.m;
            __int128 num2 = l3.c - l2.c;
            __int128 den2 = l2.m - l3.m;
...
int main() {
    ...
    CHT linecontainer;
    dp[1] = c;
    linecontainer.add(Line(b[1], dp[1]));
    for (int i = 2; i <= n; i++) {
        dp[i] = linecontainer.get(a[i]);
        linecontainer.add(Line(b[i], dp[i]));
    }
