...
int dp[1005][2][3005];
int pow_[(int) 1e3 + 5];
int recurr(int pos, bool iszero, int rem, int n, int k) {
    if(iszero and rem == 0) {
        if(pos < n) return 9LL * pow_[n - pos - 1] % mod;
        else if(pos == n) return 1;
    }
    if(pos == n) return 0;
    if(dp[pos][iszero][rem] != -1) return dp[pos][iszero][rem];
    int lim = 9;
    int ans = 0;
    for(int c = 0; c <= lim; c++) {
        //if(pos == 1) debug(c);
        int newrem = (rem + 1LL * c * pow_[pos]) % k;
        ans += recurr(pos + 1, iszero or (c > 0), newrem, n, k);
        ans %= mod;
    }
    return dp[pos][iszero][rem] = ans;
}

long long f(long long N, int k) {
    memset(dp, -1, sizeof(dp)); 
    return recurr(0, false, 0, N, k);
}
