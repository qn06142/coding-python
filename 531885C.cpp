...
#define debugArr(...)
#endif
vector<int> stor[(int) 1e5 + 5];
vector<int> storrev[(int) 1e5 + 5];
bool canvisit[(int) 1e5 + 5];
bool visited[(int) 1e5 + 5];

void dfs(int u) {
    visited[u] = true;
    for(auto i:storrev[u]) {
        if(!visited[i]) {
            dfs(i);
            canvisit[i] = true;
        }
...
int main() {
    ...
    for(int i = 1; i <= m; i++) {
        ...
        cin >> u >> v;
        stor[u].push_back(v);
        storrev[v].push_back(u);
    }
    for(int i = 1; i <= n; i++) {
        if(stor[i].empty()) continue;
        sort(stor[i].begin(), stor[i].end());
    }
    dfs(t);
    canvisit[t] = true;
    ...
    while(cur != t) {
        visited[cur] = 1;
        for(auto i:stor[cur]) {
            if(visited[i] or !canvisit[i]) continue;
            cur = i;
            break;
        }
        debug(cur);
