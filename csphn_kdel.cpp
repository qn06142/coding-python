...
int main() {
    ...
    for(int i = 1; i <= n; i++) {
        if(cnt[a[i]] == 0) {
            uniques ++;
            cnt[a[i]]++;
            stor.insert({cnt[a[i]], a[i]});
        } else {
            pair<int, int> tmp = *stor.lower_bound({cnt[a[i]], a[i]});
            cnt[a[i]]++;
            stor.erase(tmp);
            stor.insert({tmp.first + 1, a[i]});
        }
        while(uniques > k + 1) {
            if(cnt[a[j]] == 1) {
                stor.erase(stor.lower_bound({1, a[j]}));
                uniques--;
            } else {
                pair<int, int> tmp = *stor.lower_bound({cnt[a[j]], a[j]});
                stor.erase(tmp);
                stor.insert({tmp.first - 1, a[j]});
            }
            cnt[a[j]]--;
            j++;
        }
