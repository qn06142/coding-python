...
int binpow(int base, int exp, int mod = MOD) {
    int res = 1;
    base %= mod;
    while (exp) {
...
int32_t main(){
    ...
    auto run_dp = [&](int req[10]) {
        vector<int> dp(n+1, 0), ndp(n+1, 0);
        dp[0] = 1;
        for (int d = 0; d < 10; d++) {
            fill(ndp.begin(), ndp.end(), 0);
            for (int len = 0; len <= n; len++) if (dp[len]) {
                for (int k = req[d]; len + k <= n; k++) {
                    ndp[len + k] = (ndp[len + k]
                                   + dp[len] * invfact[k]) % MOD;
                }
            }
            swap(dp, ndp);
        }
    ...
    int req[10];
    for (int i = 0; i < 10; i++) req[i] = a[i];
    auto dp_all = run_dp(req);
    int req_[10];
    req_[0] = max(0LL, a[0] - 1);
    for (int i = 1; i < 10; i++) req_[i] = a[i];
    auto dp_zero = run_dp(req_);
    int ans = 0;
    for (int L = 1; L <= n; L++) {
        int total    = fact[L]   * dp_all[L]   % MOD;
        int lead= fact[L-1] * dp_zero[L-1] % MOD;
        ans = (ans + total - lead + MOD) % MOD;
    }
