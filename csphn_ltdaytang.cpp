...
signed main() {
    ...
    for(int i = 1; i < h; i++) {
        int r = i;
        while(!dq.empty() and a[dq.back()] - dq.back() <= a[r] - r) dq.pop_back();
        dq.push_back(r);
    }
    int ans = LLONG_MAX;
    for(int i = 1; i <= n - h + 1; i++) {
        ...
        int max_ = a[dq.front()] - dq.front();
        if(max_ > (1 - l)) continue;
        ans = min(ans, ((h * (h + 1)) >> 1) - (pref[r] - pref[l - 1]));
        debug(l, r);
        debug(((h * (h + 1)) /2), (pref[r] - pref[l - 1]));
    }
    if(ans == LLONG_MAX) cout << -1; else
    cout << ans << '\n';
}