...
void bit_update(int i, int v, int M) {
    for(; i <= M; i += i & -i)
        bit[i] = max(bit[i], v);
}
...
int main(){
    ...
    for(int i = 0; i < n; i++){
        a[i].first  = get_idx(a[i].first);
        a[i].second = get_idx(a[i].second);
    }
    ...
    int answer = 0;
    for(int i = 0; i < n; i++){
        int start_c = a[i].first;
        int end_c   = a[i].second;

        int best_before = bit_query(start_c);
        int dp_i = best_before + 1;

        bit_update(end_c, dp_i, M);
