...
long long digit_dp(int pos, int mod, int digit_sum_mod, bool tight, const vector<int>& digits) {
    if (pos == digits.size()) {

        return (mod == 0 && digit_sum_mod == 0) ? 1 : 0;
    }
    if (dp[pos][mod][digit_sum_mod][tight] != -1) {
        return dp[pos][mod][digit_sum_mod][tight];
    }
    ...
    for (int digit = 0; digit <= limit; ++digit) {
        res += digit_dp(
            pos + 1,
            (mod * 10 + digit) % K,             
            (digit_sum_mod + digit) % K,        
            tight && (digit == limit),          
            digits
        );
    }

    return dp[pos][mod][digit_sum_mod][tight] = res;
}

long long count_valid_numbers(long long X) {
    if (X < 0) return 0; 
    vector<int> digits = get_digits(X);
    memset(dp, -1, sizeof(dp));
    return digit_dp(0, 0, 0, 1, digits); 
}
...
int main() {
    ...
    long long result = count_valid_numbers(B) - count_valid_numbers(A - 1);
