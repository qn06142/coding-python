...
void operator+=(pair<int, int>& a, const pair<int, int>& b) {
    if (a.first > b.first) {
        ...
        a = {a.first, (a.second + b.second) % mod};
    }
}
void solve() {
    string s, t;
    ...
    s = " " + s;
    t = " " + t;
    vector<vector<int>> next1(n + 5, vector<int>(26)), next2(m + 5, vector<int>(26));
    fill(next1[n + 1].begin(), next1[n + 1].end(), n + 1);
    for(int i = n; i >= 1; i--) {
        for(int c = 0; c < 26; c++) {
    ...
    fill(next2[m + 1].begin(), next2[m + 1].end(), m + 1);
    for(int i = m; i >= 1; i--) {
        for(int c = 0; c < 26; c++) {
    ...
    vector<vector<pair<int, int>>> dp(n + 5, vector<pair<int, int>>(m + 5));
    for(int i = n; i >= 1; i--) {
        for(int j = m; j >= 1; j--) {
            if(s[i] != t[j]) continue;
            dp[i][j] = {1, 1};
            for(int c = 0; c < 26; c++) {
                if(next1[i + 1][c] == n + 1 or next2[j + 1][c] == m + 1) continue;
                dp[i][j] += {dp[next1[i + 1][c]][next2[j + 1][c]].first + 1, dp[next1[i + 1][c]][next2[j + 1][c]].second};
            }
        }
    }
    debug(dp);
