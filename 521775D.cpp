...
void remove(int x) {
    ...
    if (curFreq > 0) arr[curFreq]--;  
    cnt[x]--;  
    if (cnt[x] > 0) arr[cnt[x]]++;  
}

void compress_array() {
    vector<int> sorted_a(a, a + n);
...
int main() {
    ...
    for (const auto& query : queries) {
        int ql = query.l, qr = query.r, qk = query.k;

        while (r < qr) add(compressed_a[++r]);
        while (r > qr) remove(compressed_a[r--]);

        while (l < ql) remove(compressed_a[l++]);
        while (l > ql) add(compressed_a[--l]);

        ans[query.idx] = arr[qk];
    }
