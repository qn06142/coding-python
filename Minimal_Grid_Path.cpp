...
int main() {
    ...
    vector<pair<int,int>> layer_;
    layer_.emplace_back(0, 0);
    ...
    for (int dist = 1; dist <= 2 * n - 2; ++dist) {
        ...
        for (auto p : layer_) {
            ...
            if (r + 1 < n && seen[r+1][c] != time) {
                seen[r+1][c] = time;
                next_.emplace_back(r+1, c);
                tar = min(tar, a[r+1][c]);
            }
            if (c + 1 < n && seen[r][c+1] != time) {
                seen[r][c+1] = time;
                next_.emplace_back(r, c+1);
                tar = min(tar, a[r][c+1]);
            }
        }
        ...
        layer_.clear();
        for (auto p : next_) {
            if (a[p.first][p.second] == tar) layer_.push_back(p);
        }
