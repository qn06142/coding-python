...
int digit_dp(int pos, int even, int odd, bool bound) {
    if (pos < 0) {
        if (even > odd) {
            return is_prime[even - odd];  
        }
        return 0;
    }
    if (dp[pos][even][odd][bound] != -1) {
        return dp[pos][even][odd][bound];
    }
    ...
    for (int c = 0; c <= newlim; c++) {
        if (pos & 1) {
            ...
            ans += digit_dp(pos - 1, even + c, odd, bound && (c == newlim));
        } else {

            ans += digit_dp(pos - 1, even, odd + c, bound && (c == newlim));
        }
    }

    return dp[pos][even][odd][bound] = ans;
}
