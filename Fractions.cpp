...
int main() {
    ...
    for (int i = 1; i <= n; i++) {
        ...
        InvM.mat[0][1] = 1;
        InvM.mat[1][0] = 1;
        InvM.mat[1][1] = (MOD - (a[i] % MOD)) % MOD;
        p[i] = multiply(p[i - 1], M);
        invp[i] = multiply(InvM, invp[i - 1]); 
    }

    while (q--) {
        int l, r;
        cin >> l >> r;
        Matrix Res = multiply(invp[l - 1], p[r]);
        cout << Res.mat[0][0] << " " << Res.mat[1][0] << "\n";
    }
}