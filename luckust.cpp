...
const int blocks = 32;
const int maxn = 1005;
vector<int> stor[blocks + 5][blocks + 5];
...
int main() {
    ...
    for(int l = 1; l * blocks <= n; l++) {
        stor[l][l - 1].resize(m + 5, 0);
        for(int r = l; r * blocks <= n; r++) {

            stor[l][r] = stor[l][r - 1];
            if(r == l) {
                calc(r * blocks, r * blocks, stor[l][r]);
            } else {
                calc((r - 1) * blocks + 1, r * blocks, stor[l][r]);
            }
        }
    }
    ...
    while(q--) {
        ...
        cin >> l >> r;
        int lp = -1, rp = -1;
        for (int i = 1; i * blocks <= r; ++i) {
            if (l <= i * blocks && i * blocks <= r) {
                if (lp == -1) lp = i;
                rp = i;
            }
        }
        ...
        if(lp == -1) {
            ...
            calc(l, lp * blocks - 1, tmp);
            calc(rp * blocks + 1, r, tmp);
        }
        cout << tmp[m] << '\n';
    }
}
