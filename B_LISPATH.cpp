...
void dfs(int node, int parent) {
    int pos = lower_bound(currentLIS.begin(), currentLIS.end(), a[node]) - currentLIS.begin();
    int prevValue = -1;
    if (pos < currentLIS.size()) {
        prevValue = currentLIS[pos];
        currentLIS[pos] = a[node];
    } else {
        currentLIS.push_back(a[node]);
    }
    
    lis[node] = currentLIS.size();
