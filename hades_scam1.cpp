...
signed main() {
    ...
    for(int i = 1; i <= n; i++) {
        int pre = LLONG_MAX, suf = LLONG_MAX;
        for(int h = 1; h <= maxh; h++) {
            pre = min(pre, dp[i - 1][h] - c * h);
            if(a[i] <= h)
            dp[i][h] = calc(a[i], h) + pre + c * h;
        }
        for(int h = maxh; h >= 1; h--) {
            suf = min(suf, dp[i - 1][h] + c * h);
            if(a[i] <= h)
            dp[i][h] = min(dp[i][h], calc(a[i], h) + suf - c * h);
        }
    }
