...
struct bit_tree {
    int n;
    vector<int> bit;
    bit_tree(int _n): n(_n), bit(n+1, 0) {}
...
int main(){
    ...
    int Q;
    cin >> Q;
    vector<pair<char,long long>> ops(Q);
    ...
    for (auto &qr : ops) {
        char tp = qr.first;
        long long v = qr.second;

        if (tp == 'I') {
            int r = int(lower_bound(comp.begin(), comp.end(), v) - comp.begin()) + 1;

            if (bit.query(r) - bit.query(r-1) == 0)
                bit.update(r, +1);
        }
        else if (tp == 'D') {
            int r = int(lower_bound(comp.begin(), comp.end(), v) - comp.begin()) + 1;
            ...
            int r = int(lower_bound(comp.begin(), comp.end(), v) - comp.begin());
