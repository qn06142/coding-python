...
map<int, vector<pair<int, int>>> points_by_diag;
map<int, map<int, int>> fw;
map<int, map<int, int>> bw;
int maxsize;
...
vector<int> lis(vector<pair<int, int>> a) {
...
vector<int> lds(vector<pair<int, int>> a) {
    reverse(a.begin(), a.end());
    a.emplace_back(0, 0);
    reverse(a.begin(), a.end());
    fenwick f(0, maxsize);
...
int main() {
    ...
    for (int i = 1; i <= k; i++) {
        auto [x, y] = a[i];
        a[i] = {lower_bound(cmp.begin(), cmp.end(), x) - cmp.begin() + 1,
                lower_bound(cmp.begin(), cmp.end(), y) - cmp.begin() + 1};
    }

    n = lower_bound(cmp.begin(), cmp.end(), n) - cmp.begin() + 1;
    m = lower_bound(cmp.begin(), cmp.end(), m) - cmp.begin() + 1;

    for (int i = 1; i <= k; i++) {
        auto [x, y] = a[i];
        points_by_diag[x + y].emplace_back(x, y);
        points.emplace_back(x, y);
    }

    for (auto &i : points_by_diag) {
    ...
    for (pair<int, int> i : points) {
        maxpath = max(maxpath, len(i.first, i.second));
    }

    int ans = 0;
    for (const auto &i : points_by_diag) {
        int max_ = 0;
        int cnt = 0;
        for (pair<int, int> j : i.second) {
            int len_ = len(j.first, j.second);
            if (len_ > max_) {
                cnt = 1;
                max_ = len_;
            } else if (len_ == max_) {
