...
long long countsep(vector<Rect>& R, int N){

    vector<long long> C_end(N+2, 0), C_start(N+2, 0);
    for(auto &r: R){
        C_end[r.x2]++;
        C_start[r.x1]++;
    }

    vector<long long> S_start(N+3, 0);
    for(int x = N; x >= 0; x--){
        S_start[x] = S_start[x+1] + C_start[x+1];
    }

    long long Sx = 0;
    for(int x = 0; x <= N; x++){
        Sx = (Sx + C_end[x] * S_start[x]) % MOD;
    }

    fill(C_end.begin(), C_end.end(), 0);
    fill(C_start.begin(), C_start.end(), 0);
    for(auto &r: R){
        C_end[r.y2]++;
        C_start[r.y1]++;
    }
    for(int y = N; y >= 0; y--){
        S_start[y] = S_start[y+1] + C_start[y+1];
    }
    long long Sy = 0;
    for(int y = 0; y <= N; y++){
        Sy = (Sy + C_end[y] * S_start[y]) % MOD;
    }

    return (Sx + Sy) % MOD;
}

vector<Rect> countrect(const vector<vector<int>>& A){
    int N = A.size() - 1;
    vector<int> h(N+1);
    vector<Rect> out;
    out.reserve(N*N*N);  
    ...
    for(int i = 1; i <= N; i++){
        ...
        for(int j = 1; j <= N; j++){
            ...
            while(!st.empty() && st.back().first >= h[j]){
                auto [h0, s0] = st.back(); st.pop_back();

                for(int left = s0; left < j; left++){
                    int top = i - h0 + 1;
                    out.push_back({top, left, i, j-1});
                }
                start = s0;
            }
            if(h[j] > 0) st.emplace_back(h[j], start);
        }
