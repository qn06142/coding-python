...
int main() {
    ...
    while(change) {
        ...
        for(int i = 1; i <= q; i++) {
            if(l[i] > r[i]) continue;
            change = true;
            t[(l[i] + r[i]) / 2].push_back(i);
        }
        dsu tmp;
        for (int edge = 1; edge <= m; edge++) {
            tmp.unite(e[edge].first, e[edge].second);
            for (int qi : t[edge]) {
                if (tmp.find(q_[qi].first) == tmp.find(q_[qi].second)) {
                    ans[qi] = edge;
                    r[qi] = edge - 1;
                } else {
                    l[qi] = edge + 1;
    ...
    for(int i = 1; i <= q; i ++) {
        cout << (q_[i].first == q_[i].second ? 0 : ans[i]) << '\n';
    }
}