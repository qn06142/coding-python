...
int main(){
    ...
    for(auto &p : qs){
        ...
        long long double_fact = fact[2*n] * invfact[n] % MOD;
        long long inv_pow2n = modpow(pow2[n], MOD-2);
        double_fact = double_fact * inv_pow2n % MOD;
        long long comb_ = comb(n, k);
        long long ans = comb_ * double_fact % MOD;
        cout << ans << '\n';
    }
}