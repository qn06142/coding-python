...
int main() {
    ...
    // a[i] = number of stones at distance i from the near bank (1 ≤ i ≤ w-1)
    vector<long long> a(w, 0);
    for (int i = 1; i <= w-1; i++) {
        cin >> a[i];
    }

    // prefix_sum[i] = sum of a[1] through a[i]
    vector<long long> prefix_sum(w, 0);
    for (int i = 1; i < w; i++) {
        prefix_sum[i] = prefix_sum[i-1] + a[i];
    }

    // slide a window of length l over positions 1..w-1
    // window ending at i covers stones in [i-l+1 .. i]
    long long answer = LLONG_MAX;
    for (int end = l; end <= w-1; end++) {
        long long window_sum = prefix_sum[end] - prefix_sum[end - l];
