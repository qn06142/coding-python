...
int d[(int) 1e5 + 5];
int n;
__int128_t hashs[(int) 1e6 + 5];
__int128_t powmod[(int) 1e6 + 5];
const __int128_t mod = 2305843009213693951;
const __int128_t modsquared = (mod * mod);
const int base = 373;
void calcpow() {
    powmod[0] = 1;
    for(int i = 1; i <= 1e6; i++) {
...
void calchash() {
    for(int i = 1; i <= n; i++) {
        hashs[i] = (hashs[i - 1] * base + (__int128_t) d[i]) % mod;
    }
}

int calchash(int l, int r) {
    return ((__int128_t) hashs[r] - (__int128_t) hashs[l - 1] * powmod[r - l + 1] + modsquared) % mod;
}

bool check(int len) {
    map<__int128_t, int> m;
    for(int i = 1; i <= n - len + 1; ++i) {
        __int128_t hash_val = calchash(i, i + len - 2);
        if(m.find(hash_val) != m.end()) {
            return true;
        }
        m[hash_val] = 1;
    }
