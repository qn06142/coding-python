...
int main() {
    int n;
    cin >> n;
    vector<int> a(n + 1), right_effect(n + 1, 1), left_effect(n + 1, 1);

    for (int i = 1; i <= n; i++) cin >> a[i];
    for (int i = n; i >= 1; i--) {
        int reach = min(n, i + a[i] - 1);
        int extra = 0;
        for(int j = i + 1; j <= reach; j++) {
            extra = max(extra, min(n, j + a[j] - 1));
            right_effect[i]++;
        }
        for(int j = )
    }
    ...
    for (int i = 1; i <= n; i++) {
        int reach = max(1, i - a[i] + 1);
        if (reach < i) {
            left_effect[i] += left_effect[i - 1];
        }
    }

    for (int i = 1; i <= n; i++) cout << left_effect[i] << " ";
    cout << "\n";
    for (int i = 1; i <= n; i++) cout << right_effect[i] << " ";
}
