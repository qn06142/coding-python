...
int main(){
    ...
    while (true) {
        ...
        seen[cur] = (int)seq.size();
        seq.push_back(cur);

        for (int i = 0; i < (int)n; ++i) {
            int j = (i + 1) % (int)n;
            if (cur[i] == cur[j]) continue;
            char c = third_char(cur[i], cur[j]);
            cur[i] = c;
            cur[j] = c;
        }
    }
    debug(seen.size());
    if ((long long)seq.size() > k) {
        cout << seq[k] << '\n';
        return 0;
    } else {
        int start = cycle_start;
        int len = (int)seq.size() - start;
        long long idx = start + (k - start) % len;
        cout << seq[idx] << '\n';
        return 0;
    }
}