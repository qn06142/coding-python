...
signed main() {
    ...
    for (int i = 1; i <= n; ++i) {
        int maxa = 0;
        for(int j = 3; j <= n; j++) {
            maxa = max(maxa, a[i][j] + a[i][j - 1] + a[i][j - 2]);
            f[i][j][1] = maxa;
        }
    }
    ...
    for (int turn = 1; turn <= k; ++turn) {
        ...
        for (int i = 1; i <= n; ++i) {
            int increase = f[i][n][cnt[i] ^ 1] - f[i][n][cnt[i] & 1]; 
            if (increase > maxIncrease) {
                maxIncrease = increase;
                it = i;
            }
        }
        assert(maxIncrease != -1);
        ans += maxIncrease;
        debug(maxIncrease);
        debug(it);
        cnt[it] ^= 1;
        int x = cnt[it];
        for (int j = 3; j <= n; ++j) {
            f[it][j][x ^ 1] = max(f[it][j - 1][x ^ 1], f[it][j - 3][x] + a[it][j] + a[it][j - 1] + a[it][j - 2]);
        }
    }
    for(int i = 1; i <= n; i++) {
        debug(f[i][n]);
