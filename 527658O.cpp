...
void generate(string current, int open1, int close1, int open2, int close2, int open3, int close3, int depth) {
    debug(current);
    if (current.length() == n and isValid(current)) {
        if (current == S) {
            cout << rank_count + 1 << endl;
            exit(0);
        }
        rank_count++;
        return;
    ...
    if(depth > k) return;
    if (open1 < n / 2) {
        generate(current + '(', open1 + 1, close1, open2, close2, open3, close3, depth + 1);
    }
    if (close1 < open1) {
        generate(current + ')', open1, close1 + 1, open2, close2, open3, close3, depth);
    }
    if (open2 < n / 2) {
        generate(current + '[', open1, close1, open2 + 1, close2, open3, close3, depth + 1);
    }
    if (close2 < open2) {
        generate(current + ']', open1, close1, open2, close2 + 1, open3, close3, depth);
    }
    if (open3 < n / 2) {
        generate(current + '{', open1, close1, open2, close2, open3 + 1, close3, depth + 1);
    }
    if (close3 < open3) {
        generate(current + '}', open1, close1, open2, close2, open3, close3 + 1, depth);
    }
}
