...
const int maxn = 1005;
int a[maxn], b[maxn];
long long c[maxn * maxn], d[maxn * maxn], c_size = 0, d_size = 0;
int d_pref[maxn * maxn];
int n, m, k;
bool check(long long x) { // does x has at most k elems that is greater than x
    int j = d_size;
    int cnt = 0;
    for(int i = 1; i <= c_size; i++) {
        while(c[i] + d[j] >= x and j >= 1) j--;
        cnt += d_size - j;
    }
    return cnt >= k;
}
signed main() {
    ...
    //assert(ans != -1);
    int res = 0;
    // calc sum of elem that satisfies: c[i] + d[j] > ans
    // <=> d[j] > ans - c[i]
    debug(d_pref);
    debug(ans);
    for(int i = 1; i <= c_size; i++) {
        int j = upper_bound(d + 1, d + 1 + d_size, ans - c[i]) - d;
        k -= d_size - j + 1;
        res += d_pref[d_size] - d_pref[j - 1] + c[i] * (d_size - j + 1);
        debug(c[i], j, d_pref[d_size] - d_pref[j - 1],  c[i] * (d_size - j + 1));
    }
    res += k * ans;
    cout << res << '\n';
    return 0;
