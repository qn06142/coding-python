...
int digitDP(int pos, int tight, int sum, const vector<int>& digits) {
    if (pos == digits.size()) {
        return sum; 
    }
    ...
    for (int digit = 0; digit <= limit; digit++) {
        result += digitDP(pos + 1, tight && (digit == limit), sum + digit, digits);
    }
...
signed main() {
    ...
    while(q--) {
        ...
        ll result = digitSum(b) - digitSum(a - 1);
        cout << result << endl;
    }
    return 0;
}