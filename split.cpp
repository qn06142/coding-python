...
struct ind {
    ...
    const inline bool operator<(const ind& b) const {
        if (y - x != b.y - b.x) {
            return (y - x) < (b.y - b.x);
        }
        return (match[x][b.x] < y - x + 1) && (s[x + match[x][b.x]] < s[b.x + match[x][b.x]]);
    }

    const inline bool operator==(const ind& b) const {
        return (y - x == b.y - b.x) && (match[x][b.x] >= y - x + 1);
    }
};
...
int main() {
    ...
    for (int len = 1; len <= n; ++len) {
        for (int i = 0; i < n; ++i) {
            ...
            if (l + len - 1 <= n) {
                int r = l + len - 1;
                if (s[l] == '0' && l < r) continue;
                stor.push_back({l, r});
            }
        }
    }
    int ans = 0;
    for(int i = 0; i < (int)stor.size(); i++) {
        int j = i;
        while(j < (int)stor.size() and stor[i] == stor[j]) j++;
        list<pair<int, int>> stor1;
        for(int l = i; l < j; l++) {
            ind x = stor[l];
            int cur = query(x.x - 1) + 1;
            ans = max(ans, cur);
            stor1.push_back({x.y, cur});
        }
        for(auto i:stor1) {
