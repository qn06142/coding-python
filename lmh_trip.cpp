...
int n;
vector<int> g[MAXN+1];
int tin[MAXN+1], tout[MAXN+1], depth[MAXN+1], timer;
int up[17+1][MAXN+1];        
int LOG;
...
int main(){
    ...
    for(int i = 1; i <= n; i++){
        ...
        if(multiples.empty() || multiples[0] != 1)
        ...
        stk.clear();
        stk.push_back(multiples[0]);
        long long tot_weight = 0;

        auto add_edge = [&](int u, int v){
            int w = lca(u,v);

            tot_weight += (depth[u] + depth[v] - 2LL*depth[w]);
        };
