...
int main() {
    ...
    int offset = 2 * n;
    int maxIndex = 4 * n + 5;
    vector<long long> cnt(maxIndex, 0);
    ...
    long long ans = 0;
    for (int a_idx = 0; a_idx <= n-1; ++a_idx) {

        ans += cnt[pref[a_idx] + offset];

        int remove_pos = a_idx + 1;
        if (remove_pos <= N) {
            cnt[pref[remove_pos] + offset]--;
        }
        int add_pos = a_idx + n + 1;
        if (add_pos <= N) {
            cnt[pref[add_pos] + offset]++;
        }
    }
if (pref[n] == 0) ans -= (n - 1);
