...
int main() {
    ...
    for (int i = 0; i < n; ++i) {
        prefix_sum[i + 1] = prefix_sum[i] + (vowels.count(s[i]) ? 1 : 0);
    }

    long double total_beauty = 0.0;

    for (int i = 0; i < n; ++i) {
        for (int j = i; j < n; ++j) {
            int vowel_count = prefix_sum[j + 1] - prefix_sum[i];
            int length = j - i + 1;
            total_beauty += (long double)vowel_count / length;
        }
    }

    cout << fixed << setprecision(6) << total_beauty << endl;
    return 0;
}