...
using namespace std;
int a[(int) 2e3 + 5];
int dp[(int) 2e3 + 5][(1 << 15)];
const int primeind[] = {0, 0, 1, 2, 0, 3, 0, 4, 0, 0, 0, 5, 0, 6, 0, 0, 0, 7, 0, 8, 0, 0, 0, 9, 0, 0, 0, 0, 0, 10, 0, 11, 0, 0, 0, 0, 0, 12, 0, 0, 0, 13, 0, 14, 0, 0, 0, 15, 0, 0, 0, 0};
int arraytomask(array<int, 16> arr) {
    int mask = 0;
    for(int i = 1; i <= 15; i++) {
        if(arr[i])
        mask |= (1 << i);
...
array<int, 16> factorize(int x) {
    ...
    for(pair<int, int> i:factors) {
        tmp[primeind[i.first]] = i.second & 1;
    }
    return tmp;
}
int main() {
    ...
    for(int i = 1; i <= n; i++) {
        cin >> a[i];
        a[i] = arraytomask(factorize(a[i]));
    }
    dp[0][0] = 1;
    for(int i = 1; i <= n; i++) {
        for(int mask = 0; mask < (1 << 15); mask++) {
            dp[i][mask] = dp[i - 1][mask] + dp[i][a[i] ^ mask];
        }
    }
    cout << dp[n][0];
}