...
int n, k;
int a[maxn], b[maxn];
long long dp[maxn][11][11], pow_[maxn][11];
...
signed main() {
    ...
    for (int i = 1; i <= n; ++i) {
        ...
        pow_[i][0] = 1;
        for (int j = 1; j <= k; ++j) {
            pow_[i][j] = pow_[i][j - 1] * b[i];
        }
    }
	for (int i = 1; i <= n; ++i) {
        for (int open = 0; open <= k; ++open) {
            for (int close = 0; close + open <= k; ++close) {
				dp[i][open][close] = NEG_INF;
			}
		}
	}
    dp[0][0][0] = 0;
    ...
    for (int i = 1; i <= n; ++i) {
        for (int open = 0; open <= k; ++open) {
            for (int close = 0; close + open <= k; ++close) {
                dp[i][open][close] = a[i] * pow_[i][open];
                long long best = 0;
                for (int closed = 0; closed <= close; ++closed) {
                    for (int opened = 0; opened + closed <= open + close; ++opened) {
                        best = max(best, dp[i - 1][opened][closed]);
                    }
                }

                dp[i][open][close] += best;
                result = max(result, dp[i][open][close]);
            }
        }
    }
    cout << result;
