...
=======
    vector<int> prefix_max1(n + 1, -1e18); 
    vector<int> prefix_max2(n + 1, -1e18); 
    for (int i = 1; i <= n; ++i) {
        prefix_max1[i] = max(prefix_max1[i - 1], w1 * a[i]);
    }
    for(int i = 2; i <= n; i++) {
        prefix_max2[i] = max(prefix_max2[i - 1], prefix_max1[i - 1] + w2 * a[i]);
    }
    vector<int> suffix_max1(n + 2, -1e18); 
    vector<int> suffix_max2(n + 2, -1e18); 

    for (int i = n; i >= 1; --i) {
        suffix_max1[i] = max(suffix_max1[i + 1], w1 * a[i]);
    }
    for(int i = n - 1; i >= 1; i--) {
        suffix_max2[i] = max(suffix_max2[i + 1], suffix_max1[i + 1] + w2 * a[i]);
    }
    int result = -1e18;
    debug(prefix_max1);
    debug(prefix_max2);
    ...
    for (int i = 3; i <= n - 2; ++i) {
        ...
        int weight = prefix_max2[i - 1] + a[i] + suffix_max2[i + 1];
        result = max(result, weight);
>>>>>>> Stashed changes
    }
