...
void recur(int i, bool nonzero, bool zeros, string prev) {
    if (i > len / 2) {  // Base case: reached the middle of the palindrome
        if (zeros || nonzero) { // Ensure at least one non-zero or zeros allowed
            string tmp = prev;
            reverse(tmp.begin(), tmp.end());
            for(int i = 0; i <= tmp.size(); i++) {
                if(tmp[i] != '0') prev[i] = tmp[i];
            }
            stor.push_back(prev);
        }
        return;
    ...
    for (int c = 0; c <= 9; c++) {
        if (!zeros && i == 0 && c == 0) continue; // Skip leading zeros if 'zeros' is false
        prev[i] = c + '0';
        recur(i + 1, nonzero || (c != 0), zeros, prev);
    }
}
int main() {
    cin >> n >> m;
    map<int, int> rem; // palindromes for the first part, without zeros
    len = n / 2;
    string temp(len, '0');
    recur(0, 0, 0, temp);
    for(string &i : stor) {
