...
long long ver[maxn][maxn]; 

long long calcv(int i, int j, int k) {

    return ver[i][j] - ver[i - k][j];
}
...
int main() {
    ...
    for (int i = 1; i <= n; ++i) {
        for (int j = i + 1; j <= n; ++j) {
            ...
            vector<long long> tb(m + 1, 0);
            vector<long long> side(m + 1, 0); 
            for (int x = 1; x <= m; ++x) {
                long long tb_ = a[i][x] + a[j][x];
                tb[x] = tb[x-1] + tb_;
                long long col = ver[j][x] - ver[i-1][x]; 
                side[x] = col - tb_; 
            }
            ...
            if (m >= 2) {
                best = side[1] - tb[0]; 
                for (int r = 2; r <= m; ++r) {
                    long long tmp = tb[r] + side[r] + best;
                    ans = max(ans, tmp);
                    long long val = side[r] - tb[r-1];
                    best = max(best, val);
                }
            }
        }
