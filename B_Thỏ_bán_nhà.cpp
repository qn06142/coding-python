    ...
    for (int min_value : a) {
        ...
        for (int i = 1; i <= n; i++) {
            if (a[i] >= min_value) {
                ...
                for (int j = i; j <= n && a[j] >= min_value; j++) {
                    ...
                    if (j >= i + k - 1) {
                        ...
                        segment_mins.push_back(a[dq.front()]);
        ...
        if (segment_mins.size() >= q) {
            sort(segment_mins.begin(), segment_mins.end());

            int max_in_q = segment_mins[q - 1];
            int min_in_q = segment_mins[0];
            ans = min(ans, max_in_q - min_in_q);
        }
    }
