...
int a[(int) 3e5 + 5];
bool inrange(int l, int r, int i) {
    return l <= i && i <= r;
}
signed main() {
    ...
    long long bestblue = 0, currentSum = 0;
    int blueL = 0, blueR = -1;
    int tempL = 0;

    for (int i = 1; i <= n; ++i) {
        currentSum += (a[i] > 0 ? a[i] : a[i] * 3);

        if (currentSum > bestblue) {
            bestblue = currentSum;
            blueL = tempL;
            blueR = i;
        }
    ...
    bestblue = 0;
    for(int i = blueL; i <= blueR; i++) {
        bestblue += a[i];
    }
    debug(blueL, blueR, bestblue);
    debugArr(a, n + 1);
    int bestgreen = 0;
    for (int i = 1; i <= n; ++i) {
        if (!inrange(blueL, blueR, i)) {
            bestgreen += max(a[i], -a[i]);
        }
    }
    debug(bestgreen);
    cout << bestblue * 2 + bestgreen << endl;
}
