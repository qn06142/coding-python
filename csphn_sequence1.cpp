...
int pref[maxn];
int pref_[maxn];
int rm[maxn]; // rightmost position that has sum from rm[i] -> i >= k
int a_[maxn];
bool check(int m) {
    for(int i = 1; i <= n; i++) {
        pref_[i] = min(pref_[i - 1], pref[i] - m * i);
        if(rm[i] == 0) continue;
        int curr = pref[i] - m * i;
        if(curr - pref_[rm[i] - 1] >= 0) return true;
    }
    return false;
}
signed main() {
    ...
    int j = 1;
    for(int i = 1; i <= n; i++) {
        while(j < i and pref[i] - pref[j] >= k) {
            j++;
        }
        if(pref[i] - pref[j - 1] >= k) {
            rm[i] = j;
        }
    }
    debug(pref);
