...
pair<int, int> comb(pair<int, int> a, pair<int, int> b, int mod) {
    int p = modpow(10LL, b.first, mod);
    p = (p * a.second) % mod;
    p = (p + b.second) % mod;
    return make_pair((a.first + b.first) % 82260072, p);
}

vector<vector<pair<int, int>>> precompute() {
    vector<vector<pair<int, int>>> dp(10, vector<pair<int, int>>(MAX_N, make_pair(0, 0)));
    for (int i = 0; i < 10; ++i) {
        dp[i][0] = make_pair(1, i);
    }
...
signed main() {
    ...
    vector<vector<pair<int, int>>> dp = precompute();
    int t;
    cin >> t;
    ...
    while (t--) {
        ...
        pair<int, int> res = dp[k.back() - '0'][n];
        for (int i = k.length() - 2; i >= 0; --i) {
            res = comb(dp[k[i] - '0'][n], res, MOD);
        }
        cout << res.second << '\n';
    }
