...
int recur(int pos, int lastdig, bool tight, bool iszero) {
    if(pos < 0) {
        return 1;
    }
    if(dp[pos][lastdig][tight][iszero] != -1) return dp[pos][lastdig][tight][iszero];
    int lim = (tight ? a[pos] : 9);
    int ans = 0;
    for(int c = 0; c <= lim; c++) {
        if(c == lastdig and c != 0) continue;
        if(c==0 and !iszero and lastdig == 0) continue;
        ans += recur(pos - 1, c, tight and c == lim, iszero and c == 0);
    }
    return dp[pos][lastdig][tight][iszero] = ans;
}
int countValidNumbers(int num) {
    ...
    return recur(size - 1, 0, true, true);
}
...
signed main() {
    ...
    int result = countValidNumbers(b) - countValidNumbers(a - 1);
    cout << result << endl;
