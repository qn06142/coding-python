...
int recur(int pos, int balance, bool tight) {
    if (pos == n && balance == 0) {
        return 1;  
    }
    if (pos == n || balance < 0) {
    ...
    int ans = 0;
    int limit = tight ? a[pos] : 1;  

    for (int d = 0; d <= limit; d++) {
        int new_tight = (tight && (d == a[pos])) ? 1 : 0;  
        if (d == 0) {  
            ans = (ans + recur(pos + 1, balance + 1, new_tight)) % MOD;
        } else if (balance > 0) {  
            ans = (ans + recur(pos + 1, balance - 1, new_tight)) % MOD;
        }
    }

    return dp[pos][balance + n][tight] = ans;  
}
