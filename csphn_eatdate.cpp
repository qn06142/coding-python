...
long long a[MAXN], pref[MAXN], pre[MAXN];
...
int main(){
    ...
    for(int i = 1; i <= n; i++){
        pre[i] = pref[i] + (long long)(n - i) * a[i];
    }
    ...
    while(T--){
        ...
        int i = int(lower_bound(pre + 1, pre + n + 1, s) - pre);

        long long need    = s - pref[i-1];
        long long count_p = n - (i - 1);
        long long x       = (need + count_p - 1) / count_p;
