...
int recur(int pos, bool iszero, bool tight, int last) {
    if (pos == N.size()) {
        return last != -1 and iseven(last) and iszero;
    }

    if (stor[pos][iszero][tight][last] != -1) {
        return stor[pos][iszero][tight][last];
    }
    ...
    for (int digit = 0; digit <= limit; ++digit) {
        if (!iszero && !iseven(digit)) continue;
        if (pos == N.size() - 1 && !iseven(digit)) continue;

        result += recur(pos + 1, iszero or digit != 0, tight && (digit == limit), digit);
    }

    stor[pos][iszero][tight][last] = result;
    return result;
}
