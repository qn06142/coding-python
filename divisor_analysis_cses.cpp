...
pair<int, long long> a[(int) 1e5 + 5];
long long cnt[(int) 1e5 + 5];
int n;
const int mod = 1e9 + 7;
...
void countdivs() {
    long long ans = 1;
    for (int i = 1; i <= n; i++) {
        ans = ans * (a[i].second + 1) % mod;
    }
    cout << ans << ' ';
}

long long sumofpowers(int x, int k) {
    long long num = (binpow(x, k + 1, mod) - 1 + mod) % mod; 
    long long denom = binpow(x - 1, mod - 2, mod);          
...
void sumdiv() {
    long long ans = 1;
    for (int i = 1; i <= n; i++) {
        ans = ans * sumofpowers(a[i].first, a[i].second) % mod;
    }
    cout << ans << ' ';
}
...
void proddiv() {
    ...
    long long ans = 1;
    for (int i = 1; i <= n; i++) {
        ans = binpow(ans, a[i].second + 1, mod) * 
              binpow(binpow(a[i].first, a[i].second * (a[i].second + 1) / 2, mod), cnt, mod) % mod;
        cnt = cnt * (a[i].second + 1) % (mod - 1); 
    }
    cout << ans << '\n';
}
