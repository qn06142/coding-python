...
void revseg(vector<int>& p, int start, int end, int total) {

    int count = ((end - start + total) % total) + 1;
    for (int k = 0; k < count / 2; k++) {
        int i1 = (start + k) % total;
        int i2 = (end - k + total) % total;
        swap(p[i1], p[i2]);
    }
}
...
int main(){
    ...
    vector<bitset<maxn>> enemies(total + 1);
    ...
    for (int i = 1; i <= total; i++) {
        ...
        for (int j = 0; j < k; j++){
            int e;
            cin >> e;
            enemies[i].set(e, true);
            enemies[e].set(i, true); 
        }
    }
    ...
    while (changed) {
        ...
        for (int i = 0; i < total; i++) {
            ...
            if (enemies[ka].test(kb)) {

                int jind = -1;
                for (int d = 1; d <= total - 2; d++) {
                    int j = (i + 1 + d) % total;
                    int prev_j = (j - 1 + total) % total;
                    if (!enemies[ka].test(p[j]) && !enemies[kb].test(p[prev_j])) {
                        jind = j;
                        break;
                    }
                }
                ...
                if (jind != -1) {
                    ...
                    int ssta = (i + 1) % total;
                    int send = (jind - 1 + total) % total;
                    revseg(p, ssta, send, total);
                    changed = true;
                    break; 
                }
            }
