...
#define debugArr(...)
#endif
const long long NINF = LLONG_MIN/4;
...
int main(){
    ...
    vector<pair<long long,int>> brow(used+1, {NINF, -1});
    ...
    for(int i = 0; i < n; ){
        ...
        for(int t = i; t < j; t++){
            ...
            if(id == 1){
                ...
                auto [best, from_id] = brow[P.y];
                if(best >= k){
                    long long cand = best - k + P.f;
                    if(cand > dp[id]){
                        dp[id] = cand;
                        parent[id] = from_id;
                    }
        ...
        for(int t = i; t < j; t++){
            ...
            if(bcol >= k){
                long long cand = bcol - k + P.f;
                if(cand > dp[id]){
                    dp[id] = cand;
                    parent[id] = best_col_id;
                }
            }
