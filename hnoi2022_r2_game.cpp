...
int main() {
    ...
    for (int i = 0; i < N; ++i) {
        int energy = m[i], experience = e[i], reduction = s[i];

        for (int j = W; j >= energy; --j) {
            int tries = 0;
            int exptot = 0;
            while(experience - reduction * tries >= 0) {
                debug(energy, tries);
                if(j < (energy * tries)) break;
                dp[j] = max(dp[j], dp[j - (energy * tries)] + exptot);
                exptot += experience - reduction * (tries);
                tries++;
            }
        }
    }
