...
const int maxn = 20;
int a[maxn];
void bacctracc(int i, int n, long long s, vector<long long>& ans) {
    if(i > n){ 
        ans.push_back(s);
        return;
    }
    bacctracc(i + 1, n, s + a[i] * 2, ans);
    bacctracc(i + 1, n, s + a[i], ans);
    bacctracc(i + 1, n, s, ans);
}
int main() {
    ...
    while(t--) {
        ...
        int mid = n / 2;
        bacctracc(1, n / 2, 0, l);
        bacctracc(n / 2 + 1, n, 0, r);
        debug(l, r);
        sort(l.begin(), l.end());
        sort(r.begin(), r.end());
        bool found = false;
        for(long long x : r) {
            if(binary_search(l.begin(), l.end(), k - x)) {
                cout << "Yes\n";
                found = true;
                break;
            }
