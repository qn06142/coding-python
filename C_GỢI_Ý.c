...
Node* newnode(void) {
    ...
    node->best_freq = 0;
    node->best_id = -1;
    for (int i = 0; i < ALPHA; i++) node->nxt[i] = NULL;
    return node;
}

void dfs(Node* node) {

    node->best_freq = node->cnt;
    node->best_id = node->leaf_id;

    for (int c = 0; c < ALPHA; c++) {
        Node* child = node->nxt[c];
        if (!child) continue;
        dfs(child);

        if (child->best_freq > node->best_freq ||
            (child->best_freq == node->best_freq &&
             (node->best_id == -1 || child->best_id < node->best_id))) {
            node->best_freq = child->best_freq;
            node->best_id = child->best_id;
        }
    }
}
...
int main(void) {
    ...
    char **distinct = malloc(n * sizeof *distinct);
    int *freq = malloc(n * sizeof *freq);
    int dcnt = 0;
    for (int i = 0; i < n; i++) {
        if (i == 0 || strcmp(words[i], words[i-1]) != 0) {
            distinct[dcnt] = words[i];
            freq[dcnt] = 1;
            dcnt++;
        } else {
            freq[dcnt - 1]++;
    ...
    for (int id = 0; id < dcnt; id++) {
        const char *w = distinct[id];
        Node* cur = root;

        for (int j = 0; w[j]; j++) {
            int c = w[j] - 'a';
            if (c < 0 || c >= ALPHA) break;
            if (!cur->nxt[c]) cur->nxt[c] = newnode();
            cur = cur->nxt[c];
        }
    ...
    while (q--) {
        scanf("%10s", buf);
        Node* cur = root;
        for (int i = 0; buf[i] && cur; i++) {
            int c = buf[i] - 'a';
            if (c < 0 || c >= ALPHA) { cur = NULL; break; }
            cur = cur->nxt[c];
        }
        if (!cur || cur->best_id < 0) {
            puts("-1");
        } else {
            printf("%s %d\n", distinct[cur->best_id], cur->best_freq);
        }
    }
}