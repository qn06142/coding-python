...
bool visited[maxn];
int comps = 0;
void dfs(int u, int c, vector<int>& nodes) {
    visited[u] = true;
    comp[u] = c;
    nodes.push_back(u);
    for (int v : adj[u]) {
...
int main() {
    ...
    vector<vector<int>> bydeg(n); 

    for (int c = 1; c <= comps; c++) {
        vector<int> tmp;
        for (int u : components[c - 1]) {
            tmp.push_back(deg[u]);
        }
    ...
    for (int d = 0; d < n; d++) {
        if (bydeg[d].empty()) continue;
        vector<int> sizes;
        for (int c_id : bydeg[d]) {
            sizes.push_back(sz[c_id]);
        }
        sort(sizes.rbegin(), sizes.rend());
        int sum = 0;
        int take = min((int)sizes.size(), k);
