...
signed main() {
    ...
    for(int i = 1; i <= n; i++) {
        for(int j = i + 1; j <= n; j++) {
            ...
            while(l <= r) {
                int mid = (l + r) / 2;
                if(isacute(a[i].first, a[j].first, a[mid].first)) {
                    l = mid + 1;
                    res = mid;
                } else r = mid - 1;
            }
            debug(a[i].second, a[j].second, pref[res] - pref[j]);
            if(res != -1)
            ans += a[i].second * a[j].second * (pref[res] - pref[j]);
        }
    }
    cout << ans << '\n';
}