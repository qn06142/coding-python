...
int maxsum(const std::vector<int>& a) {
    int n = a.size() - 1;
    vector<int> pref(n + 1), minpref(n + 1);
    for(int i = 1; i <= n; i++) {
        pref[i] = pref[i - 1] + a[i];
    ...
    int ans = INT_MIN;
    for(int i = 2; i <= n; i++) {
        ans = max(ans, pref[i] - minpref[i - 2]);
    }
    return ans;
}
signed main() {
    ...
    vector<int> sumpairs(n + 1);
    for(int i = 1; i <= n; i++) {
        sumpairs[i] = a[i] + a[i - 1];
    }
    vector<int> arr;
    arr.push_back(0);
    for(int i = 2; i <= n; i += 2) {
        arr.push_back(sumpairs[i]);
    }
    int ans = maxsum(arr);
    debug(arr);
    arr.clear();
