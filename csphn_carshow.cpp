...
bool check(long long x1, long long v1, long long x2, long long v2, long long t) {

    if (v1 <= v2) return false;

    __int128 l = (__int128)x2 - (__int128)x1;
    __int128 r = (__int128)(v1 - v2) * (__int128)t;
    return l <= r;
}
...
int main() {
    ...
    for (auto &cv : a) {
        long long xi = cv.first;
        long long vi = cv.second;
        if (ans.empty()) {
            ans.emplace_back(xi, vi);
            continue;
        }
        auto [hx, hv] = ans.back();
        if (check(xi, vi, hx, hv, t)) {
