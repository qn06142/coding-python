...
int n, k, m;
priority_queue<transaction> a, b, c;

book stor[(int) 1e6 + 5];
bool cmp(const book &a, const book &b) {
    return a.b < b.b;
}
int first_choice() { //choose a[i] that is smallest into the basket
    while(!a.empty() and stor[a.top().bookid].sel) {
        a.pop();
    }
    if(a.empty()) return LLONG_MAX;
    else return a.top().weight;
}
int second_choice() { // choose ith book that has a[i] - b[i] smallest and j so that b[j] is min
    while(!b.empty() and stor[b.top().bookid].sel) {
        b.pop();
    }
    if(b.empty() or c.empty()) return LLONG_MAX;
    else return b.top().weight + c.top().weight;
}
signed main() {
    ...
    cin >> n >> k >> m;
    for(int i = 1; i <= n; i++) {
        cin >> stor[i].a >> stor[i].b;
    }
    debug(k);
    sort(stor + 1, stor + 1 + n, cmp);
    for(int i = 1; i <= n; i++) {
        a.push({i, stor[i].a});
        b.push({i, stor[i].b});
    }
    for(int i  =1; i <= k; i++) {
        if(stor[i].b <= m) {
            stor[i].sel = true;
            c.push({i, stor[i].a - stor[i].b});
            m -= stor[i].b;
        }
    }
    //assert(!c.empty());
    while(true) {
        int cost1 = first_choice();
        int cost2 = second_choice();
        if(min(cost1, cost2) > m) // broke ahh man
        break;
        if(cost1 < cost2) {
            int id = a.top().bookid;
            a.pop();
            //c.push({id, stor[id].a - stor[id].b});
            stor[id].sel = true;
            m -= cost1;
        } else {/*
            stor[c.top().bookid].sel = false;
            int i = c.top().bookid;
            a.push({i, stor[i].a});
            b.push({i, stor[i].b});*/
            c.pop();
    ...
    /*
    for(int i = 1; i <= n; i++) {
        if(!stor[i].sel) stor[i].a = stor[i].b = 0;
    }
    while(!c.empty()) {
        int id = c.top().bookid;
        c.pop();
