    ...
    for(int i = 1; i <= n + m; i++) {
        for(int x = 0; x <= t; x++) {
            for(int y = 0; y <= t; y++) {
                ...
                if(x >= a[i]) {
                    // smol cart will deliver the ith package
                    if(i <= n) // we can only deliver smol
                    dp[i & 1][x][y] = max(dp[i & 1][x][y], dp[(i + 1) & 1][x - a[i]][y] + 1);
                }
                if(y >= a[i]) {
                    // OOOFFFM cart does the job
                    dp[i & 1][x][y] = max(dp[i & 1][x][y], dp[(i + 1) & 1][x][y - a[i]] + 1);
                }
            }
        }
    }
