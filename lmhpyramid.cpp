...
int main() {
    ...
    vector<vector<int>> prefmod(26, vector<int>(length + 1, 0));

    for (int i = 0; i < length; ++i) {
        int mind = char_map(s[i]);
        if (mind != -1) {
            prefmod[mind][i + 1]++;
        }
    }
    ...
    while (q--) {
        ...
        if (t == 1) {
            ...
            ll count = full_repeats * prefmod[mind][length];

            if (start_mod <= end_mod) {
                count += prefmod[mind][end_mod] - prefmod[mind][start_mod - 1];
            } else {
                count += prefmod[mind][length] - prefmod[mind][start_mod - 1];
                count += prefmod[mind][end_mod];
            }
