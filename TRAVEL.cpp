...
namespace Subtask1 {
    ...
    void backtrack(int l, int r, int wt, int val, vector<pair<int,int>> &vec) {
        if(l > r) {
            vec.emplace_back(wt, val);
            return;
        }

        backtrack(l + 1, r, wt + w[l], val + v[l], vec);

        backtrack(l + 1, r, wt, val, vec);
    }
    void solve() {
        int mid = n / 2;
        backtrack(1, mid, 0, 0, s1);
        backtrack(mid + 1, n, 0, 0, s2);
