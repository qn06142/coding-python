...
pair<int, int> digitDP(int pos, int tight, int sum, const vector<int>& digits) {
    if (pos == digits.size()) {
        return {0, 1}; 
    }
    if (dp[pos][tight] != make_pair(-1, -1)) {
        return dp[pos][tight];
    }
    ...
    for (int digit = 0; digit <= limit; digit++) {
        pair<int, int> ans = digitDP(pos + 1, tight && (digit == limit), sum + digit, digits);
        result = result + make_pair(ans.second * digit + ans.first, ans.second);
    }
...
int digitSum(int x) {
    ...
    return digitDP(0, 1, 0, digits).first;
}
...
signed main() {
    ...
    int result = digitSum(b) - digitSum(a - 1);
    cout << result << endl;
    return 0;
}