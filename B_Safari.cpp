...
struct stor {
    pair<int, int> coords, time;
    const bool operator<(const stor &other) const {
        return time.second < other.time.second;
    }
};
stor a[5005];
signed main() {
    ...
    for (int i = 1; i <= n; i++) {
        cin >> a[i].coords.first >> a[i].coords.second;
    }
    for (int i = 1; i <= n; i++) {
        cin >> a[i].time.first >> a[i].time.second;
    }
    sort(a + 1, a + n + 1);
    ...
    for (int i = 1; i <= n; i++) {
        for (int j = 0; j < i; j++) {
            ...
            int arriveTime = a[j].time.second + dist(a[j].coords, a[i].coords);

            int val = a[i].time.second - max(arriveTime, a[i].time.first);
            if(val >= 0) {
