...
void toBase3(int num, int n, vector<int>& base3Mask) {
    for (int i = 0; i < n; ++i) {
        base3Mask[n - 1 - i] = num % 3;
        num /= 3;
    }
}
...
int main() {
    ...
    while (t--) {
        ...
        for (int mask = 0; mask < tot; ++mask) {
            vector<int> base3Mask(n);
            toBase3(mask, n, base3Mask);

            long long as = 0, bs = 0;
            for (int i = 0; i < n; ++i) {
                if (base3Mask[i] == 0) {
                    as += a[i]; 
                } else if (base3Mask[i] == 1) {
                    bs += b[i]; 
                } else if (base3Mask[i] == 2) {
                    as += a[i]; 
                    bs += b[i]; 
                }
            }
