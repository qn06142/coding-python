...
int a[2005][2005];
long long pref[2006][2006];
long long colp[2006][2006];
long long diagp[2006][2006];
int main() {
    ...
    for (int i = 1; i <= n; ++i) {
        for (int j = 1; j <= m; ++j) {
            ...
            colp[i][j] = colp[i - 1][j] + a[i][j];
        }
    }
    for(int i = -m; i <= m; i++) {
        for(int j = 0; j < n; j++) {
            if(i + j >= 1 and i + j <= m)
            diagp[1 + j][i + j] = diagp[j][i + j - 1] + a[1 + j][i + j];
        }
    }
    int ans = 0;
    for(int i = k; i <= n; i++) {
        ...
        for (int x = 0; x < k; ++x) {
            ...
            int row = i - k + 1 + x; 
            int leftCol = j - k + 1; 
            int rightCol = j - k + 1 + x; 
            curr += pref[row][rightCol] - pref[row][leftCol - 1] 
                        - pref[row - 1][rightCol] + pref[row - 1][leftCol - 1];
        }
        ans = max(ans, curr);
        for(int j = k + 1; j <= m; j++) {
            debug(i - k, j, i - k, j - k);
            curr -= (colp[i][j - k] - colp[i - k][j - k]);
            curr += (diagp[i][j] - diagp[i - k][j - k]);
            ans = max(ans, curr);
        }
    }
    cout << ans << '\n';
