...
int main(){
    ...
    debug(a_);
    for (auto& p : stor) {
        arr.push_back({p.first, p.second, (int)arr.size() + 1, 0, -1});
    }
    sort(arr.begin(), arr.end(), [](const ele &x, const ele &y){
        if (x.a == y.a)
            return x.b > y.b;
        return x.a < y.a;
    });
    ...
    for (int j = 0; j < n; j++){
        int r = lower_bound(all.begin(), all.end(), arr[j].b) - all.begin() + 1;
        arr[j].ni = m - r + 1;
    }

    vector<int> bit(m + 1, 0), bit_idx(m + 1, -1);
    ...
    auto qry = [&](int pos) -> pair<int, int> {
        int best = 0, best_idx = -1;
        for (; pos; pos -= pos & -pos) {
            if (bit[pos] > best) {
                best = bit[pos];
                best_idx = bit_idx[pos];
            }
        }
    ...
    vector<int> seq;
    for (int j = best_i; j != -1; j = arr[j].par)
        seq.push_back(a_[arr[j].a]);
