...
signed main() {
    ...
    memset(prefm, -0x3f, sizeof prefm);
    dp[0][0] = 0;
    for(int i = 0; i <= n; i++) {
        prefm[i][0] = 0;
    }
    for(int k = 1; k <= 3; k++) {
        int max_ = -0x3f3f3f3f3f3f3f3f;
        for(int i = 1; i <= n; i++) {
            max_ = max(max_, prefm[i - 1][k - 1] - pref[i - 1]);
            dp[i][k] = max(dp[i][k], pref[i] + max_);
            prefm[i][k] = max(prefm[i - 1][k], dp[i][k]);
        }
    }
    debug(dp);
    debug(prefm);
