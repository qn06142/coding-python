...
struct Matrix {
    ...
    Matrix(int n_ = 0) : n(n_), a(n_, vector<long long>(n_, INF)) {}
...
Matrix multiply(const Matrix &A, const Matrix &B) {
    ...
    for (int i = 0; i < n; ++i) {
        for (int t = 0; t < n; ++t) {
            if (A.a[i][t] == INF) continue;
            long long v = A.a[i][t];
            for (int j = 0; j < n; ++j) {
                if (B.a[t][j] == INF) continue;
                long long candidate = v + B.a[t][j];
                if (candidate < R.a[i][j]) R.a[i][j] = candidate;
            }
        }
    }
    return R;
