...
void dfs(int v, const vector<vector<int>> &adj, vector<int> &order, vector<char> &visited) {
    visited[v] = 1;
    for (int u : adj[v]) if (!visited[u]) dfs(u, adj, order, visited);
    order.push_back(v);
}
...
int main() {
    ...
    for (int i = 1; i <= n; ++i) if (!visited[i]) dfs(i, adj, order, visited);

    vector<vector<int>> adj_rev(n + 1);
    for (int i = 1; i <= n; ++i) for (int u : adj[i]) adj_rev[u].push_back(i);
    ...
    for (int i = 1; i <= n; ++i) {
        for (int u : adj[i]) {
            if (roots[i] != roots[u]) cond[roots[i]].push_back(roots[u]);
        }
    }
    for (int r = 1; r <= n; ++r) {
        if (!cond[r].empty()) {
            sort(cond[r].begin(), cond[r].end());
            cond[r].erase(unique(cond[r].begin(), cond[r].end()), cond[r].end());
        }
    }

    vector<int> indeg(n + 1, 0);
    for (int r = 1; r <= n; ++r) {
        for (int v : cond[r]) indeg[v]++;
    }
    ...
    long long ans = 0;
    for (int r = 1; r <= n; ++r) if (roots[r] == r) ans = max(ans, dp[r]);
    cout << ans << "\n";
}