...
struct Line { long long a,b; Line(long long A=0,long long B=LLONG_MAX):a(A),b(B){} long long eval(long long x)const{return a*x+b;} };

struct LiChao {
    int n; vector<Line> seg; vector<long long> xs;
    LiChao(vector<long long> v){ sort(v.begin(),v.end()); v.erase(unique(v.begin(),v.end()),v.end()); xs=v; n=xs.size(); seg.assign(4*n+5, Line()); }
    void add(Line nw,int idx,int l,int r){
        int m=(l+r)>>1;
        long long xl=xs[l], xm=xs[m], xr=xs[r];
        Line cur=seg[idx];
        if(nw.eval(xm) < cur.eval(xm)){ seg[idx]=nw; nw=cur; }
        if(l==r) return;
        if(nw.eval(xl) < seg[idx].eval(xl)) add(nw, idx<<1, l, m);
        else if(nw.eval(xr) < seg[idx].eval(xr)) add(nw, idx<<1|1, m+1, r);
    }
    void add_line(long long a,long long b){ if(n>0) add(Line(a,b),1,0,n-1); }
    long long query_pos(int p,int idx,int l,int r) const {
        long long res = seg[idx].eval(xs[p]);
        if(l==r) return res;
        int m=(l+r)>>1;
        return min(res, p<=m ? query_pos(p,idx<<1,l,m) : query_pos(p,idx<<1|1,m+1,r));
    }
    long long query(long long x) const {
        int p = lower_bound(xs.begin(), xs.end(), x) - xs.begin();
        return query_pos(p,1,0,n-1);
    }
};
...
int main(){
    ...
    int m; cin>>m; vector<long long> q(m), xs;
    for(int i=0;i<m;i++){ cin>>q[i]; xs.push_back(q[i]); }
    LiChao T(xs);
    for(auto &ln:L) T.add_line(ln.first, ln.second);
    for(int i=0;i<m;i++) cout<<T.query(q[i])<<"\n";
}
