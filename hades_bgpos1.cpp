...
{
    scanf("%li",&n);
    F[n+1] = 10000*100000 + 1;  // Có 100000 số 10000
    for (long i=1;i<=n;i++) {
        scanf("%li",&a[i]);
        S = S + a[i];  // Tính tổng
        tmp += a[i];
        F[n+1] = min(F[n+1],tmp); // Tính F[n+1]
    }
    if (S <= 0) {  // kiểu nào cũng chết
        printf("0");
        return 0;
    }
    if (F[n+1] > 0) res++; // F[1] tôtt
    for (long i=n;i>=2;i--) { // không tính F[1]
        F[i] = min(F[i+1]+a[i],a[i]);
        res += (F[i] > 0);  // tốt thì cộng
    }
    printf("%li",res);
    debug(F);
    return 0;
