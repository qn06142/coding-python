...
int dp[1005][2][2][1005]; // dp[i][tight][validzero][count]
int a[1005];
int target_digit;
int target_count;

int recur(int i, bool tight, bool validzero, int count) {
    if (count > target_count) return 0;
    if (i < 0) return (count == target_count);
    if (!tight && dp[i][tight][validzero][count] != -1)
        return dp[i][tight][validzero][count];

    int newlim = (tight ? a[i] : 9);
    int res = 0;
    dp[i][tight][validzero][count] = 0;

    res %= 1000000007;
    for (int digit = 0; digit <= newlim; digit++) {
        int new_count = count;
        if (validzero || digit > 0) {
            if (digit == target_digit) {
                new_count++;
            }
        } else if (digit == target_digit && target_digit == 0) {
            new_count++;
        }
        if (new_count <= target_count) {
            res += recur(i - 1, tight && (digit == a[i]),
                         (validzero || (digit > 0)), new_count) % 1000000007;
        }
        res %= 1000000007;
    }
    res %= 1000000007;

    if (!tight) dp[i][tight][validzero][count] = res;
    return res;
}
...
int main() {
    ...
    string n_str;
    cin >> n_str >> target_digit >> target_count;
