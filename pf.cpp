...
int count_occurrences(const string& s, int len) {
    long long prefix_hash = get_prefix_hash(1, len);
    int count = 0;
    for (int i = 1; i + len - 1 <= n; i++) {
        if (get_prefix_hash(i, i + len - 1) == prefix_hash) {
            count++;
        }
    }
    return count;
...
int main() {
    ...
    for (int len = 1; len <= n; len++) {
        if (get_prefix_hash(1, len) == get_prefix_hash(n - len + 1, n)) {
            int freq = ans[len];
            stor.emplace_back(len, freq);
        }
    }
