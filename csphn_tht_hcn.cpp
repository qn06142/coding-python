...
Point a[MAXN];
vector<int> x_coordinates[MAXN];
vector<int> unique_x;
int stor[2 * MAXN][2 * MAXN];
...
int main() {
    ...
    for (int i = 1; i <= n; i++) {
        a[i].x = lower_bound(compress.begin(), compress.end(), a[i].x) - compress.begin();
        a[i].y = lower_bound(compress.begin(), compress.end(), a[i].y) - compress.begin();
    }
    sort(a + 1, a + 1 + n);
    ...
    for(int i = 1; i <= n; i++) {
        for(int j = i + 1; j <= n; j++) {
            if(a[i].x != a[j].x and a[i].y != a[j].y and a[i].x != a[j].y and a[i].y != a[j].x)
            if(stor[a[i].x][a[j].y] and stor[a[j].x][a[i].y]) {
                ans += (stor[a[i].x][a[i].y] * stor[a[i].x][a[j].y] * stor[a[j].x][a[i].y] * stor[a[j].x][a[j].y]);
            }
        }
    }
    cout << ans;
