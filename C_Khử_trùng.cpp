...
long long get_elem(int x) { return get(x) - get(x - 1); }
long long get_range(int l, int r) { return get(r) - get(l - 1); }
...
void decrement(int l, int r) {
    ...
    while (it != active.end() && *it <= r) {
        int ind = *it;
        ++it; 
        long long newv = a[ind] / k;
        if (newv != a[ind]) {
            long long delta = newv - a[ind];
            update(ind, delta);
            a[ind] = newv;
            if (newv == 0) todel.push_back(ind);
        }
    }
    for (int i : todel) active.erase(i);
}

void set_elem(int idx, long long b) {
    long long v = a[idx];
    if (v == 0 && b != 0) active.insert(idx);
    if (v != 0 && b == 0) active.erase(idx);
    update(idx, b - v);
    a[idx] = b;
}
