...
int inverse(int a, int m) {
    return a <= 1 ? a
                  : m - (long long)(m / a) * inverse(m % a, m) % m;
}
...
int main(){
    ...
    int M = 1;
    while (M <= 2*maxv + off * 2) M <<= 1;
    vector<int> freq(M, 0);
    vector<int> freq_(M, 0);
    for (int x : a) {
        freq[x]++;
        freq_[x]++;
    }
    fft(freq, false);
    for (int i = 0; i < M; i++)
        freq[i] = (int)((long long)freq[i] * freq[i] % mod);
    fft(freq, true);
    int zero = freq_[off];
    for (int v = 0; v <= maxv; v++) {
        int pos = 2*v;
        freq[pos] = (freq[pos] - freq_[v] + mod) % mod;
    }
    debug(freq);
    int ans = 0;
    for(int i = 0; i < n; i++) {
