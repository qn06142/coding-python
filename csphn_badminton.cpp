...
bool beat(int i, int j) {
    return a[i].first > b[i].first and a[i].second > b[i].second;
}
struct person {
    int x, y;
    bool col;
...
int main() {
    ...
    vector<person> stor;
    for(int i = 1; i <= n; i++) {
        stor.emplace_back(a[i].first, a[i].second, 0);
        stor.emplace_back(b[i].first, b[i].second, 1);
    }
sort(stor.begin(), stor.end());
    ...
    for(person i : stor) {
        if(i.col) {
            ...
            multiset<int>::iterator it = stor1.lower_bound(i.y);
            if(it == stor1.begin()) continue;
            ans++;
            stor1.erase(--it);
        }
    }
