...
namespace sub1 {
    bool check() {
        return *max_element(a + 1, a + 1 + n) - *min_element(a + 1, a + 1 + n) == 0;
    }
    void solve() {
        int col = b[a[1]];
        sort(b + 1, b + 1 + n);
        int pos = upper_bound(b + 1, b + 1 + n, col) - b;
        int ans = 0;
        debug(pos);
        for(int i = pos; i <= m; i++) {
            if(k == 1) {
...
signed main() {
    ...
    cin.tie(0);
    #ifdef ONLINE_JUDGE
    freopen("CMA.INP", "r", stdin);
    freopen("CMA.OUT", "w", stdout);
    #endif
    cin >> n >> m >> k;
    for(int i = 1; i <= n; i++) {
        cin >> a[i];
