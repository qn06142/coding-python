...
def generate_test_case():
    """
    Generate a random test case for the double palindrome problem.
    Ensures N is even (1 <= N <= 20) and M is in the range (1 <= M <= 1_000_000).
    Returns:
        str: Test case as a string "N M"
    """
    N = random.choice([i for i in range(2, 21, 2)])  # Even N between 2 and 20
    M = random.randint(1, 10)  # Random M in range 1 to 1_000_000
    return f"{N} {M}"
...
def main():
    executable1 = ['python', 'palin_cc.py']
    executable2 = './palin_cc'
