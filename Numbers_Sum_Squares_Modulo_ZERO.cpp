...
#define debugArr(...)
#endif
//      i  tight validzero  sum
int dp[10][2]   [2]        [10005]; // count of numbers with ^ conditions
int a[10];
int recur(int i, bool tight, bool validzero, int sum) {
    //debug(i, tight, validzero, sum);
    if(i < 0) return (sum % 3 == 0);
    //debug(' ');
    if(!(tight) and dp[i][tight][validzero][sum] != -1) return dp[i][tight][validzero][sum];
    int newlim = (tight ? a[i] : 9);
    int res = 0;
    dp[i][tight][validzero][sum] = 0;
    for(int c = 0; c <= newlim; c++) {
        res += recur(i - 1, tight and (c == a[i]), (validzero or (c > 0)), (sum + c * c) % 3);
    }
    if(!(tight)) dp[i][tight][validzero][sum] = res;
    return res;
}
int f(int x) {
    int n = 0;
