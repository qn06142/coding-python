...
static int board[(1 << 10) + 5][(1 << 10) + 5];
int tile_id = 1;

int getquad(const coord &tl, const coord &lr, const coord &p) {
    int size = lr.x - tl.x + 1;
    int half = size / 2;
    int midx = tl.x + half;
    int midy = tl.y + half;
    if (p.x < midx && p.y < midy) return 1;   
    if (p.x < midx && p.y >= midy) return 2;  
    if (p.x >= midx && p.y < midy) return 3;  
    return 4;                                 
}

void place(const coord &tl, const coord &lr, const coord &p) {
    int size = lr.x - tl.x + 1;
    if (size == 1) return;
    ...
    int midx = tl.x + half;
    int midy = tl.y + half;
    int quad = getquad(tl, lr, p);

    int cur_id = tile_id++;
    if (quad != 1) board[midx - 1][midy - 1] = cur_id;
    if (quad != 2) board[midx - 1][midy]     = cur_id;
    if (quad != 3) board[midx][midy - 1]     = cur_id;
    if (quad != 4) board[midx][midy]         = cur_id;

    if (quad == 1)
        place(tl, coord(midx - 1, midy - 1), p);
    else
        place(tl, coord(midx - 1, midy - 1), coord(midx - 1, midy - 1));

    if (quad == 2)
        place(coord(tl.x, midy), coord(midx - 1, lr.y), p);
    else
        place(coord(tl.x, midy), coord(midx - 1, lr.y), coord(midx - 1, midy));

    if (quad == 3)
        place(coord(midx, tl.y), coord(lr.x, midy - 1), p);
    else
        place(coord(midx, tl.y), coord(lr.x, midy - 1), coord(midx, midy - 1));

    if (quad == 4)
        place(coord(midx, midy), lr, p);
    else
        place(coord(midx, midy), lr, coord(midx, midy));
}
...
int main() {
    ...
    place(coord(1, 1), coord(N, N), coord(x, y));
