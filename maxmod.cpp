...
void bacctracc1(int i = 1, int s = 0) {
    if(i > n / 2) {
        stor1.push_back(s);
        return;
    }
    bacctracc1(i + 1, s);
    bacctracc1(i + 1, (s + a[i]) % m);
}
void bacctracc2(int i = n / 2 + 1, int s = 0) {
    if(i > n ) {
        stor2.push_back(s);
        return;
    }
    bacctracc2(i + 1, s);
    bacctracc2(i + 1, (s + a[i]) % m);
}
signed main() {
    cin >> n >> m;
    for(int i = 1; i <= n; i++) {
    ...
    bacctracc1();
    bacctracc2();
    sort(stor2.begin(), stor2.end());
    debug(stor1);
    debug(stor2);
    int ans = 0;
    for(int i :stor1) {
        auto it = (lower_bound(stor2.begin(), stor2.end(), m - i));
        int add = (it == stor2.begin() ? 0 : *prev(it));
        debug(i, add);
        ans = max(ans, max(i, (i + add) % m));
    }
    ans = max(ans, *max_element(stor2.begin(), stor2.end()));
    cout << ans;
}