...
map<pair<int, int>, vector<tornado>> stor;
vector<pair<int, int>> pos;
int n, m, q, k;
int a[505][505];
int ans[505][505];

vector<int> calcdiffarr(int x, int y) {
    vector<int> diff(1005);
    for(tornado i:stor[{x, y}]) {
        diff[i.r2] += i.w;
        if(i.r1 <= 1005) {
            diff[i.r1 + 1] -= i.w;
        }
...
signed main() {
    ...
    for(int i = 1; i <= k; i++) {
        int w, r1, r2, x, y;
        cin >> w >> r1 >> r2 >> x >> y;
        stor[{x, y}].push_back({w, r1, r2});
    }
    for(int d = 1; d <= q; d++) {
        auto [x, y] = pos[d - 1];
        vector<int> stor = calcdiffarr(x, y);
        for(int i = 1; i <= n; i++) {
            for(int j = 1; j <= m; j++) {
                if(dist(i, j, x, y) <= 1004) {
                    ans[i][j] += stor[dist(i, j, x, y)];
                }
            }
        }
    }
