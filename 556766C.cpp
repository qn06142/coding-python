...
bool check(int mid) {
    debug(mid);
    vector<int> ind;
    long long total_skepticism = 0;

    for(int i = 1; i <= n; i++) {
        if(a[i].first > mid) {  
            int skepticism_increase = (a[i].first - mid) * a[i].second;
            ind.push_back(skepticism_increase);
        }
    }

    sort(ind.begin(), ind.end());

    int clubs_to_modify = ind.size() - x + 1;
    if(clubs_to_modify > ind.size()) return true;
    for(int i = 0; i < clubs_to_modify; i++) {
        total_skepticism += ind[i];
        if(total_skepticism >= k) return false;  
    }
