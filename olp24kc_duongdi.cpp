...
void extenedeuclidian(int a, int b, long long &x, long long &y) {
    if (b == 0) {
        x = 1;
        y = 0;
        return;
    ...
    long long x1, y1;
    extenedeuclidian(b, a % b, x1, y1);
    x = y1;
    y = x1 - (a / b) * y1;
}

int combin(int u, int v) {
    int b = ((long long)(fact[u]) * fact[v - u]) % MOD;
    long long x, y;
    extenedeuclidian(b, MOD, x, y);
    x = (x + MOD) % MOD; 
    return ((long long)(fact[v]) * x) % MOD;
}
...
signed main() {
    ...
    dp[1][1] = 1;
    for (int i = 1; i <= min(1000LL, m); i++) {
        for (int j = 1; j <= min(1000LL, n); j++) {
            if (!bl[i][j - 1] && j > 1) {
    ...
    if (m > 1000 || n > 1000) {
        ...
        for (int i = 1; i <= min(1000LL, m); i++) {
            if (!bl[i][1000]) {
                int paths_from_edge = dp[i][1000];
                int remaining_rows = m - i;
                int remaining_cols = n - 1001;
                debug(remaining_rows, remaining_cols);
                result = (result + paths_from_edge * combin(remaining_rows, remaining_rows + remaining_cols)) % MOD;
            }
        }
        if(m > 1000)
        for (int j = 1; j <= min(1000LL, n); j++) {
            if (!bl[1000][j]) {
                int paths_from_edge = dp[1000][j];
                int remaining_rows = m - 1001;
