...
#define debugArr(...)
#endif
pair<int, vector<int>> calc(vector<int> t) {
    int s = accumulate(t.begin() + 1, t.end(), 0);
    vector<int> dp(s * 2 + 1);
    vector<int> dp1(s * 2 + 1);
    dp[s] = true;
    for(int i = 1; i <= t.size() - 1; i++) {
        dp1 = dp;
        for(int j = -s; j <= s; j++) {
            if(j + s + t[i] <= s)
            dp[j + s] = dp1[j + s] | dp1[j + s + t[i]];
            if(j + s - t[i] >= 0)
            dp[j + s] = dp[j + s] | dp1[j + s - t[i]];
        }
    }
    return {s, dp};
}
...
int main() {
    ...
    auto [s1, dp1] = calc(storx);
    auto [s2, dp2] = calc(story);
    debug(s1);
    debug(s2);
    debug(storx);
    debug(story);
    ...
    while(q--) {
        ...
        cin >> x >> y;
        x -= storx[0];
        if(x + s1 >= dp1.size() or y + s2 >= dp2.size() or x + s1 < 0 or y + s2 < 0) {
            cout << "No\n";
            continue;
        }
        debug(x, y);
        
        if(dp1[s1 + x] and dp2[s2 + y]) {
            cout << "Yes\n";
        } else {
            cout << "No\n";
        }
