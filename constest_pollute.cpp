...
int main(){
    ...
    for(int i = d; i < n; i++){
        ...
        if(d % 2 == 1){
            ...
            auto it = window.find_by_order(d/2);
            med1 = 2LL * it->first;
        } else {

            auto it1 = window.find_by_order(d/2 - 1);
            auto it2 = window.find_by_order(d/2);
            med1 = (long long)it1->first + it2->first;
        }

        if((long long)a[i] >= med1){
            ans++;
        }

        window.erase({a[i-d], i-d});
        window.insert({a[i], i});
    }
