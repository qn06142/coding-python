...
int main(){
    ...
    cin >> s;
    int n = s.size();
    vector<vector<int>> pip(n);
    for (int j = 0; j < n; j++) {
        pip[j] = prefix_function(s.substr(j));
    }
    ...
    for(int i = 1; i <= n; i++){
        for(int j = 0; j < i; j++){
            int len = i - j;
            const vector<int> &pi = pip[j];
            int p = len - pi[len-1];
            int rep = (len % p == 0 ? len / p : 1);
            int len_ = (len % p == 0 ? p       : len);
            int cost = count_digits(rep) + len_;
