...
int nxt[(int) 3e7 + 5];
int cnt[(int) 3e7 + 5];
long long calcs(long long i, long long mod) {
    if(i % 2 == 0) {
        return (i / 2) % mod * (i + 1)% mod;
    }
    return i % mod * ((i + 1) / 2) % mod;
}

long long calcr(long long l, long long r, long long mod) {
    return (calcs(r, mod) - calcs(l - 1, mod) + mod) % mod;
}
signed main() {
    #ifdef ONLINE_JUDGE
    ...
    int cyclesize = 1;
    for(int i = (x * beg + y) % m; i != beg; i = nxt[i]) {
        cyclesize++;
    }
    int loners = 0;
    int ind = x;
    ...
    ind = 1;
    for(int i = 1; i <= m; i++) {
        ans += (calcr(ind, ind + cnt[i] - 1, m) * i) % m;
        ind += cnt[i];
        ans %= m;
    }
    cout << ans % m;
