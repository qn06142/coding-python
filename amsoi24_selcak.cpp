...
struct st {
    int s, d, p;
    st() {}
    st(int s, int d, int p) : s(s), d(d), p(p) {}
};
st a[maxn];
st b[maxn];
int main() {
    #ifdef ONLINE_JUDGE
    freopen("SELCAK.INP", "r", stdin);
    freopen("SELCAK.OUT", "w", stdout);
    #endif
    int n, m;
    cin >> n >> m;
    for(int i = 1; i <= n; i++) {
    ...
    for(int i = 1; i <= n; i++) {
        ...
        for(int k = 0; k <= n * 50; k++) {
            if(k >= a[i].s)
            f[i][k] = min(f[i][k], f[i - 1][k - a[i].s] + a[i].p);
    ...
    for(int i = 1; i <= m; i++) {
        ...
        for(int k = 0; k <= 50 * m; k++) {
            if(k >= b[i].s)
            g[i][k] = max(g[i][k], g[i - 1][k - b[i].s] + b[i].p);
