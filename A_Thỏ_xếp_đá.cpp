...
int main() {
    ...
    for (int i = 1; i <= n; i++) {
        ...
        visited[{p[i].first, p[i].second}] = 1; 
    }
    ...
    for (int i = 1; i <= n; i++) {
        for (int k = 0; k < 4; k++) { 
            int x1 = p[i].first + dx[k];
            int y1 = p[i].second + dy[k];

            if (visited[{x1, y1}] == 0) {
                q.push({p[i].first, p[i].second}); 
                ans[{p[i].first, p[i].second}] = {x1, y1}; 
                visited[{p[i].first, p[i].second}] = 2; 
                break; 
            }
        }
    }
    ...
    for (int i = 1; i <= n; i++) {
        cout << ans[{p[i].first, p[i].second}].first << ' ' 
